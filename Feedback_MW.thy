theory Feedback_MW
imports SPF SBTheorie ParComp_MW StreamCase_Study
...
lemma spfdelay_mono[simp] : "monofun 
                           (\<lambda> i. (\<lambda> sb. (sbDom\<cdot>sb = {ch3}) \<leadsto> ([ch2\<mapsto>delay\<cdot>((Y i) . ch3)]\<Omega>)))"
sorry

lemma delay_chain[simp]: "chain Y \<Longrightarrow> sbDom\<cdot>(Y 0) = {ch1,ch2} 
...
lemma delay_chain_lub[simp]: "chain Y \<Longrightarrow> sbDom\<cdot>(Lub Y) = {ch1,ch2} 
                                \<Longrightarrow> chain (\<lambda> i. [ch2\<mapsto>delay\<cdot>((Y i) . ch3)]\<Omega>)"
  by (simp add: sbChain_dom_eq2)

lemma sdelay_Lub[simp]: "chain Y \<Longrightarrow> sbDom\<cdot>(Lub Y) = {ch1,ch2} \<Longrightarrow> 
  (\<Squnion>i. add\<cdot>(Y i . ch1)\<cdot>(Y i . ch2)) = add\<cdot>((Lub Y) . ch1)\<cdot>((Lub Y). ch2)"
sorry

lemma spfdelay_chain [simp]: "chain Y \<Longrightarrow>
      chain (\<lambda> i. (\<lambda> sb. (sbDom\<cdot>sb = {ch3}) \<leadsto> ([ch2\<mapsto>delay\<cdot>((Y i) . ch3)]\<Omega>)))"
sorry
