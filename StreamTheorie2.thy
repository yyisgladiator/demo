...
lemma sconc_sdom: "sdom (s1\<bullet>s2) \<subseteq> sdom s1 \<union> sdom s2"
apply(cases "#s1=\<infinity>")
apply simp
using ninf2Fin sdom_sconc2un by blast

lemma sntimes_sdom1[simp]: "sdom (sntimes n s) \<subseteq> sdom s"
proof (induction n)
  case 0 thus ?case by simp
next
...
oops(*
apply(rule admI)
apply(auto simp add: lub_eq_Inter Inter_is_lub)
...
lemma adm_subsetEq_rc [simp]: "adm (\<lambda>s. g\<cdot>s \<subseteq> cs)"oops
...
lemma sdom_adm2[simp]: "adm (\<lambda>a. sdom (g\<cdot>a) \<subseteq> sdom a)"
oops
(*apply(rule admI)
...
lemma sdom_sconc[simp]: "sdom x \<subseteq> sdom (x \<bullet> xs)"
by (simp add: sdom_mono)
(* by (metis minimal monofun_cfun_arg sconc_snd_empty set_cpo_simps(1)) *) 

lemma "#s<\<infinity> \<Longrightarrow> sdom (s\<bullet>x) = sdom s \<union> sdom x"
using infI sdom_sconc2un by fastforce

(* repeating a stream doesn't add elements to the domain *)
lemma sinftimes_sdom[simp]: "sdom (sinftimes s) \<subseteq> sdom s"
apply(rule ind [of _s])
apply (rule admI)
apply (metis inf_chainl4 l42 set_eq_subset sinf_inf)
apply simp
apply simp
apply(subst sinftimes_unfold)
apply(subst sdom_sconc2un)
...
lemma sinf_sdom [simp]: "sdom (s\<infinity>) = sdom s"
oops
(* by (metis antisym_conv sdom_sconc sinftimes_sdom sinftimes_unfold) *)

(* sfilter doesn't add elements to the domain *)
lemma sbfilter_sbdom[simp]: "sdom (sfilter A\<cdot>s) \<subseteq> sdom s"
oops
...
lemma smap_sdom_range [simp]: "sdom (smap f\<cdot>s) \<subseteq> range f"
by (smt mem_Collect_eq range_eqI sdom_def slen_smap smap_snth_lemma subsetI)

(* every element produced by (smap f) is in the image of the function f *)
lemma smap_sdom: "sdom (smap f\<cdot>s) =  f ` sdom s"
apply(rule)
apply (smt image_eqI mem_Collect_eq sdom_def slen_smap smap_snth_lemma subsetI)
by (smt image_subset_iff mem_Collect_eq sdom_def slen_smap smap_snth_lemma)


(* Lemmas für SB *)
(* if the stream a is a prefix of the stream b then a's domain is a subset of b's *)
lemma f1 [simp]: "a \<sqsubseteq> b \<Longrightarrow> sdom a \<subseteq> sdom b"
using approxl3 sdom_sconc by blast

(* the lub of a chain of streams contains any elements contained in any stream in the chain *)
lemma l4: "chain S \<Longrightarrow> sdom (S i) \<subseteq> sdom (\<Squnion> j. S j)"
...
lemma l403: "chain S \<Longrightarrow> \<forall>i. S i \<sqsubseteq> s \<Longrightarrow> \<forall>i. sdom (S i) \<subseteq> sdom s"
by (simp add: f1)

lemma l404: "chain S \<Longrightarrow>  \<forall>i. S i \<sqsubseteq> s \<Longrightarrow> sdom (\<Squnion> j. S j) \<subseteq> sdom s"
using f1 lub_below by blast

(* streams appearing later in the chain S contain the elements of preceding streams *)
lemma l405: "chain S \<Longrightarrow> i \<le> j \<Longrightarrow> sdom (S i) \<subseteq> sdom (S j)"
by (simp add: f1 po_class.chain_mono)

lemma l43: "chain S \<Longrightarrow> finite_chain S \<Longrightarrow> sdom (\<Squnion> j. S j) \<subseteq> (\<Union>i. sdom (S i))"
using l42 by fastforce


(*wichtig*)
(* the lub doesn't have any elements that don't appear somewhere in the chain *)
lemma sdom_lub: "chain S \<Longrightarrow> sdom (\<Squnion> j. S j) = (\<Union>i. sdom (S i))"
oops
(*apply (simp add: contlub_cfun_arg)
by (simp add: lub_eq_Union)*)

text {*Sei i in N ein index der Kette S von Strömen und B eine Menge von Nachrichten. *}
lemma l44: assumes "chain S" and "\<forall>i. sdom (S i) \<subseteq> B"
  shows "sdom (\<Squnion> j. S j) \<subseteq> B"
oops
(*by (metis (mono_tags, lifting) UN_E assms sdom_lub subsetCE subsetI)*)


lemma l6: "chain S \<Longrightarrow> \<forall>i. sdom (S i) \<subseteq> B \<Longrightarrow> sdom (\<Squnion> j. S (j + (SOME k. A))) \<subseteq> B"
oops
(*by (simp add: l44 lub_range_shift) *)

(* dropping elements can't increase the domain *)
lemma sdrop_sdom[simp]: "sdom (sdrop n\<cdot>s)\<subseteq>sdom s"
...
lemma sfilter_sdom_eps: "sdom s \<inter> A = {} \<Longrightarrow> (A \<ominus> s) = \<epsilon>"
...
lemma stakewhile_dom[simp]:assumes "\<not>f a"
  shows "a\<notin>sdom (stakewhile f\<cdot>s)"
by (smt assms below_antisym lnle_conv lnless_def mem_Collect_eq sdom_def snth_less stakewhile_below stakewhile_slen)
...
proof (rule ccontr)
  ...
  hence "sdom (stakewhile f\<cdot>s) = sdom s" by simp
  hence "(snth n s)\<in>sdom (stakewhile f\<cdot>s)" by (simp add: assms(2) snth2sdom)
...
(* if there's an element a in the domain of s which fails the predicate f, then stwbl will produce a
   finite result *)
lemma stwbl_fin [simp]: assumes "a\<in>sdom s" and "\<not> f a"
...
lemma sdom_sfilter1: assumes "x\<in>sdom (A\<ominus>s)" 
  shows "x\<in>A"
by (smt assms mem_Collect_eq sdom_def sfilterl7)

lemma sdom_subset: assumes "u\<noteq>\<bottom>"
  shows "sdom s\<subseteq>sdom (u && s)"
...
lemma sdom_sfilter_subset: assumes "u\<noteq>\<bottom>"
  shows "sdom (A\<ominus>s)\<subseteq>sdom (A \<ominus> (u && s))"
...
lemma sdom_sfilter2: assumes  "x\<in>A"
  shows "x\<in>sdom s \<Longrightarrow> x\<in>(sdom (A \<ominus> s))"
oops (*
  apply(induction s)
    apply(rule admI)
    apply rule
...
lemma sdom_sfilter[simp]: "sdom (A\<ominus>s) = sdom s \<inter> A"
oops
(*  apply rule
...
lemma stwbl_id_help:
  shows "(\<forall>a\<in>sdom s. f a) \<longrightarrow> stwbl f\<cdot>s = s"
  apply (rule ind [of _s])
    apply(rule adm_imp)
...
lemma stwbl_id [simp]: "(\<And> a. a\<in>sdom s \<Longrightarrow> f a) \<Longrightarrow> stwbl f\<cdot>s = s"
...
lemma stwbl2stakewhile: assumes "a\<in>sdom s" and "\<not>f a"
...
proof -
  ...
  thus ?thesis
    by (smt Fin_02bot approxl2 assms(1) assms(2) bottomI lnle_def lnzero_def mem_Collect_eq sconc_snd_empty sdom_def sdrop_0 slen_empty_eq slen_rt_ile_eq split_streaml1 stakewhile_below stakewhile_noteq stakewhile_sdropwhilel1 stwbl_notEps stwbl_stakewhile surj_scons tdw ub_slen_stake) 
qed

lemma stwbl_sfoot: assumes "a\<in>sdom s" and "\<not>f a"
...
lemma adm_nsdom [simp]:  "adm (\<lambda>x. b \<notin> sdom x)"
proof (rule admI)
  fix Y
  assume as1: "chain Y" and as2: "\<forall>i. b\<notin>sdom (Y i)"
  thus "b\<notin>sdom (\<Squnion>i. Y i)"
  ...
  next
    ...
    proof -
      fix n
      obtain j where "Fin n < # (Y j)"  by (metis False Streams2.inf_chainl2 as1 inf_chainl3rf less_le) 
...
lemma strdw_filter_h: "b\<in>sdom s \<longrightarrow> lnsuc\<cdot>(#({b} \<ominus> srtdw (\<lambda>a. a \<noteq> b)\<cdot>s)) = #({b} \<ominus> s)"
proof(rule ind [of _s])
  have "adm (\<lambda>a. lnsuc\<cdot>(#({b} \<ominus> srtdw (\<lambda>a. a \<noteq> b)\<cdot>a)) = #({b} \<ominus> a))" by simp
  thus "adm (\<lambda>a. b \<in> sdom a \<longrightarrow> lnsuc\<cdot>(#({b} \<ominus> srtdw (\<lambda>a. a \<noteq> b)\<cdot>a)) = #({b} \<ominus> a))" by(simp add: adm_nsdom)
  show "b \<in> sdom \<epsilon> \<longrightarrow> lnsuc\<cdot>(#({b} \<ominus> srtdw (\<lambda>a. a \<noteq> b)\<cdot>\<epsilon>)) = #({b} \<ominus> \<epsilon>)" by simp
  fix a 
  fix s
  assume IH: " b \<in> sdom s \<longrightarrow> lnsuc\<cdot>(#({b} \<ominus> srtdw (\<lambda>a. a \<noteq> b)\<cdot>s)) = #({b} \<ominus> s)"
  show " b \<in> sdom (\<up>a \<bullet> s) \<longrightarrow> lnsuc\<cdot>(#({b} \<ominus> srtdw (\<lambda>a. a \<noteq> b)\<cdot>(\<up>a \<bullet> s))) = #({b} \<ominus> \<up>a \<bullet> s)"
  proof (cases "a=b")
  ...
  next
    ...
    hence "b \<in> sdom (\<up>a \<bullet> s) \<Longrightarrow> b\<in>sdom s" using False by auto
    thus ?thesis using IH f2 local.f1 by auto 
  qed
qed

lemma strdw_filter: "b\<in>sdom s \<Longrightarrow> lnsuc\<cdot>(#({b} \<ominus> srtdw (\<lambda>a. a \<noteq> b)\<cdot>s)) = #({b} \<ominus> s)"
by(simp add: strdw_filter_h)


lemma stwbl_filterlen[simp]: "b\<in>sdom ts \<longrightarrow> #({b} \<ominus> stwbl (\<lambda>a. a \<noteq> b)\<cdot>ts) = Fin 1"
...
lemma srtdw_conc: "b\<in>sdom ts  \<Longrightarrow> (srtdw (\<lambda>a. a \<noteq> b)\<cdot>(ts \<bullet> as)) = srtdw (\<lambda>a. a \<noteq> b)\<cdot>(ts) \<bullet> as"
...
proof -
  fix u ts as
  assume as1: "u \<noteq> \<bottom>" and as2: "(\<And>as. b \<in> sdom ts \<Longrightarrow> srtdw (\<lambda>a. a \<noteq> b)\<cdot>(ts \<bullet> as) = srtdw (\<lambda>a. a \<noteq> b)\<cdot>ts \<bullet> as)"
       and as3: "b \<in> sdom (u && ts)"
  obtain a where a_def: "updis a = u" by (metis Exh_Up as1 discr.exhaust) 
  have "a\<noteq>b \<Longrightarrow> b\<in>sdom ts" by (metis UnE a_def as3 lscons_conv sdom2un singletonD) 
...
lemma stwbl_conc[simp]: "b\<in>sdom ts \<Longrightarrow>
