...
proof - 
  ...
  assume a0: "chain Y"
  assume a1: "chain (\<lambda>i::nat. Rev (Set.filter P (inv Rev (Y i))))"
  have b0: "\<forall>y \<in> \<Inter>{x. \<exists>i. x = Set.filter P (inv Rev (Y i))}. P y"
    by (simp add: full_SetCompr_eq)
  have b1: "\<forall>y \<in> \<Inter>{x. \<exists>i. x = Set.filter P (inv Rev (Y i))}. (\<forall>i. y \<in> (inv Rev (Y i)))"
    by (simp add: full_SetCompr_eq)
  then have b2: "\<forall>y \<in> \<Inter>{x. \<exists>i. x = Set.filter P (inv Rev (Y i))}. y \<in> (inv Rev (\<Squnion>i::nat. Y i))"
    by (smt Inter_iff a0 inv_rev_rev mem_Collect_eq setrevLubEqInter)
  then have b3: "Rev (Set.filter P (inv Rev (\<Squnion>i::nat. Y i))) \<sqsubseteq>
    Rev (\<Inter>{x. \<exists>i. x = Set.filter P (inv Rev (Y i))})"
    by (simp add: b0 SetPcpo.less_set_def subset_eq)
  moreover have b4: "Rev (\<Inter>{x. \<exists>i. x = inv Rev (Rev (Set.filter P (inv Rev (Y i))))}) =
    (\<Squnion>i::nat. Rev (Set.filter P (inv Rev (Y i))))"
    apply (subst setrevLubEqInter)
    apply (simp add: a1)
    by auto
  moreover have b5: "Rev (\<Inter>{x. \<exists>i. x = inv Rev (Rev (Set.filter P (inv Rev (Y i))))}) = 
    Rev (\<Inter>{x. \<exists>i. x = Set.filter P (inv Rev (Y i))})"
...
lemma setrevfilter_mono: "monofun (\<lambda> S::'a::ufuncl set rev. Rev (Set.filter P (inv Rev S)))"
  apply (rule monofunI)
  by (metis (full_types) SetPcpo.less_set_def below_rev.elims(2) below_rev.simps inv_rev_rev 
    member_filter subset_iff)

lemma setrevfilter_cont[simp]:  "cont (\<lambda> S::'a::ufuncl set rev. Rev (Set.filter P (inv Rev S)))"
