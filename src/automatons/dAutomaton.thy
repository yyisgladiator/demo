...
proof(rule contrapos_pp,simp+)
  ...
  then obtain state where state_def: "sbLen (daStateSem automat state\<cdot>\<bottom>) \<in> len_set" and "\<forall>n \<in> len_set. sbLen (daStateSem automat state\<cdot>\<bottom>) \<le> n"
  ...
  hence "Fin k < sbLen (daNextOut automat state (Abs_sbElem None)) + sbLen (daStateSem automat (daNextState automat state (Abs_sbElem None))\<cdot> \<bottom>)"
    by (metis state_least add.commute assms(1) le2lnle leI lessequal_addition lnat_plus_suc notinfI3)
  thus "False"
    by (metis assms(2) dastatesem_final_h2 k_def leD sblen_sbconc sbtypeempty_sbecons_bot)
qed
