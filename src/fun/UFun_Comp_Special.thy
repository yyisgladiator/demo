theory UFun_Comp_Special
  imports UFun_Comp UFun_applyIn
begin
...
"ufSerCompRestricted f1 f2 \<equiv> Abs_ufun (Abs_cfun (\<lambda>x. (ubclDom\<cdot>x =  ufCompI f1 f2) \<leadsto> ((ufSerParComp f1 f2 \<rightleftharpoons> x)\<bar>ufRan\<cdot>f2)))"
...
lemma ufSerParComp_repAbs: assumes "serparcomp_well f1 f2"
  ...
  apply (simp add: ufSerParComp_cont)
  by (simp add: assms ufSerParComp_well, auto)
...
lemma serparcomp_comp_dom: assumes "serparcomp_well f1 f2" 
      and "serparcomp_well f2 f3" 
      and "serparcomp_well f1 f3"
...
proof-
  have "ufDom\<cdot>f1 \<subseteq> ufDom\<cdot>(ufSerParComp f1 f2)"
    using assms by (simp add: ufSerParComp_dom assms ufCompI_def, blast)
  hence a1:" ((ub\<bar>ufDom\<cdot>(ufSerParComp f1 f2))\<bar>ufDom\<cdot>f1) = (ub\<bar>ufDom\<cdot>f1)"
    by (simp add: inf.absorb_iff2 ubclrestrict_twice)
...
proof - 
  ...
  have h2:" \<forall> x. (ubclDom\<cdot>x =(ufDom\<cdot>f1\<union>ufDom\<cdot>f2 \<union> ufDom\<cdot>f3) - (ufRan\<cdot>f1\<union>ufRan\<cdot>f2 \<union> ufRan\<cdot>f3 ))\<longrightarrow>((ubclDom\<cdot>(f1 \<rightleftharpoons> x\<bar>ufDom\<cdot>f1)) = ufRan\<cdot>f1)"
   ...
   have c1_1:" \<forall> x. (ubclDom\<cdot>x \<noteq>(ufDom\<cdot>f1\<union>ufDom\<cdot>f2 \<union> ufDom\<cdot>f3) - (ufRan\<cdot>f1\<union>ufRan\<cdot>f2 \<union> ufRan\<cdot>f3 ))\<or>((ufSerParComp f1 (ufSerParComp f2 f3)) \<rightleftharpoons>x\<bar> ufDom\<cdot>(ufSerParComp f1 (ufSerParComp f2 f3))= (f1 \<rightleftharpoons> x\<bar>ufDom\<cdot>f1) \<uplus> ((ufSerParComp f2 f3) \<rightleftharpoons> x \<uplus> (f1 \<rightleftharpoons> x\<bar>ufDom\<cdot>f1)\<bar>ufDom\<cdot>(ufSerParComp f2 f3)))"
     by (smt serparcomp_comp_dom assms(1) assms(2) assms(3) serparcomp_well_associativity ubclrestrict_dom_id ufSerParComp_apply)
   ...
   proof-
     ...
     have a2:" ufCompI f1 (ufSerParComp f2 f3) = (ufDom\<cdot>f1\<union>ufDom\<cdot>f2 \<union> ufDom\<cdot>f3) - (ufRan\<cdot>f1\<union>ufRan\<cdot>f2 \<union> ufRan\<cdot>f3 )"
       by (smt serparcomp_comp_dom assms(1) assms(2) assms(3) serparcomp_well_associativity ufSerParComp_dom)
     then show ?thesis
...
lemma ufSerCompRestricted_cont: "cont (\<lambda>x. (ubclDom\<cdot>x =  ufCompI f1 f2) \<leadsto> ((ufSerParComp f1 f2 \<rightleftharpoons> x)\<bar>ufRan\<cdot>f2))"
  apply (rule ufun_contI)
  apply (metis below_option_def monofun_cfun_arg po_eq_conv)
  by (simp add: contlub_cfun_arg op_the_chain op_the_lub)

lemma ufSerCompRestricted_well:
  assumes " serparcomp_well f1 f2"
  shows "ufWell (\<Lambda> x  . (ubclDom\<cdot>x =  ufCompI f1 f2) \<leadsto> ((ufSerParComp f1 f2 \<rightleftharpoons> x)\<bar>ufRan\<cdot>f2))"
  apply(simp add: ufWell_def)
  apply (rule conjI)
  apply (rule_tac x="ufCompI f1 f2" in exI)
  apply (rule allI)
  ...
  by (smt assms option.discI option.sel ran2exists sup_ge2 ubrestrict_dom2 ufSerParComp_dom ufSerParComp_ran ufran_2_ubcldom2)
...
lemma ufSerCompRestricted_dom:
  ...
  apply (simp add: ufSerCompRestricted_cont)
  by (simp add: assms ufSerCompRestricted_well)

lemma ufSerCompRestricted_apply: 
  assumes "serparcomp_well f1 f2" and "ubclDom\<cdot>x = ufDom\<cdot>(ufSerCompRestricted f1 f2)"
  shows  "(ufSerCompRestricted f1 f2)\<rightleftharpoons>x = ((ufSerParComp f1 f2)\<rightleftharpoons>x)\<bar>(ufRan\<cdot>f2)"
...
lemma ufSerCompRestricted_ran: assumes "serparcomp_well f1 f2"
  ...
  apply (subst(1) ufran_least)
  apply (subst ufSerCompRestricted_apply)
  using assms apply blast
  apply (simp add: ubclrestrict_dom ufSerCompRestricted_dom ufran_2_ubcldom2 assms ufSerParComp_dom ubcldom_least_cs ufSerParComp_ran)+
  by blast
...
lemma ufcomp_serialparallel_apply: 
  assumes "serparcomp_well f1 f2"
      and "ubclDom\<cdot>ub = ufCompI f1 f2"
    shows "(f1 \<otimes> f2) \<rightleftharpoons> ub = ((f1\<rightleftharpoons>(ub\<bar>ufDom\<cdot>f1))\<uplus>(f2 \<rightleftharpoons>(ub\<uplus> (f1 \<rightleftharpoons> (ub\<bar>ufDom\<cdot>f1)))\<bar>ufDom\<cdot>f2 ))"
  apply (simp add: ufcomp_serialparallel_eq assms(1))
  by (subst ufSerParComp_apply, simp_all add: assms ufSerParComp_dom)


subsubsection \<open>ufSerCompRestricted\<close>


lemma ufcomp_serialparallel_restricted_eq: 
  assumes "serparcomp_well f1 f2"
  shows "(ufComp f1 f2) \<h> (ufRan\<cdot>f1) = ufSerCompRestricted f1 f2"
  apply (simp add: ufcomp_serialparallel_eq assms)
  apply (rule ufun_eqI)
  apply (simp add: assms ufhide_dom ufhide_apply ufSerCompRestricted_dom ufSerParComp_dom)+
  apply (simp add: assms ufSerCompRestricted_apply ufSerCompRestricted_dom ufSerParComp_dom ufSerParComp_ran)
  by (simp add: Diff_triv Un_Diff assms inf_commute)


(* fixpoint no longer needed *)
(*
...
proof -
  ...
  have out_eq:"\<And> ub. ubclDom\<cdot>ub =(ufDom\<cdot>?f) \<Longrightarrow> ?f \<rightleftharpoons> ub = ?g \<rightleftharpoons> ub"
    ...
    apply (metis (no_types, lifting) dom_eq g_ser g_sercomp_well ufSerComp_dom ufcomp_dom ufhide_dom)
