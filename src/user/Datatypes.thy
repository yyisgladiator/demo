...
@{type M_pure} on its stream. Therefore, each channel can be mapped 
to a set of messages from datatype @{type M_pure}.Such a mapping is 
described by the cMsg function. Only messages included in the cMsg 
are allowed to be transmitted on the respective channel.\<close>
...
text\<open>Only with such an assumption we can always artificially define 
an "empty" stream bundle. The possibility to have an empty stream 
bundle is important for various reasons. Beside being able to define 
"sensors" and "sinks" as SPFs, also the general composition of 
components may result in components without in or output channels. 
