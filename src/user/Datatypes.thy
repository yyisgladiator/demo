...
section \<open>Channeldatatype \label{sec:chadata}\<close>
...
text\<open>Since we want use the stream type for defining stream bundles, the message
...
define "sensors" and "sinks" as SPFs, also the general composition of components may 
...
text \<open>Since one can use different time models for components, we also have to use 
