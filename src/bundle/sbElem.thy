...
text\<open>Predicate @{const sbElem_well} is used to define sbElem and only allows 
functions with a Message on each channel that are allowed to be transmitted 
(@{const ctype}), or no function, if the channel type is empty.\<close>

text\<open>Type sbElem is can be interpreted as a Timeslice.\<close>
typedef 'c sbElem ("(_\<^sup>\<surd>)" [1000] 999) = "{f::('c \<Rightarrow> M) option. sbElem_well f}"
...
lemma sbtypeepmpty_sbenone[simp]:"chIsEmpty TYPE ('cs) 
  ...
  apply(simp add: chDom_def)
  apply(rule sbe_eqI)
  by (metis Diff_eq_empty_iff not_Some_eq Rep_sbElem mem_Collect_eq chDom_def 
      sbtypeempty_notsbewell)
...
text\<open>This function retrieves an element on channel e from the sbElem. This 
only works if Elements are allowed on channel e and channel e is also in 
type c.\<close>
...
lemma sbtypenotempty_fex[simp]:"\<not>(chIsEmpty TYPE ('cs)) 
                                \<Longrightarrow> \<exists>f. Rep_sbElem (sbe::'cs\<^sup>\<surd>) = (Some f)"
...
text\<open>This function Converts the Domain of an sbElem. This works if the Domain 
it converts to, is smaller or equal\<close>
...
lemma chIsEmpty2chIsEmpty:"chIsEmpty TYPE ('c) \<Longrightarrow> 
  ...
  by (metis (mono_tags, lifting) Int_Collect cEmpty_def chan_botsingle 
      insert_not_empty le_iff_inf mk_disjoint_insert repinrange)

lemma sbgetch_ctype: assumes "Rep (c::'e) \<in> range(Rep::'d \<Rightarrow> channel)" 
  ...
  by (metis (no_types, hide_lams) assms(1) assms(2) f_inv_into_f option.sel 
      sbElem_well.simps(2) sbegetch_def sbelemwell2fwell sbtypenotempty_fex)
...
lemma sberestrict_getch:
  ...
  apply (smt Rep_sbElem chDom_def f_inv_into_f mem_Collect_eq option.sel 
         rangeI sbElem_well.elims(1) sbElem_well.simps(2) subset_iff)
  by simp
  

text\<open>This unites two sbElems. It works, if type e is a subset of the union of 
...
lemma sbeunion_getchfst:
  ...
  apply (smt assms(2) sbElem_well.simps(2) Un_iff assms(1) assms(3) 
         chIsEmpty2chIsEmpty chan_eq repinrange sbgetch_ctype subset_eq)
