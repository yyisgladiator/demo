theory USpec_UFunComp
  imports USpec inc.CPOFix fun.UFun_Comp_Special
begin
...
"uspec_compwell S1 S2 \<equiv> \<forall> f1 \<in> (Rep_fst_uspec S1). \<forall> f2 \<in> (Rep_fst_uspec S2). comp_well f1 f2"
...
let Comp_set = {ufComp f1 f2 | f1 f2. f1\<in>(Rep_fst_uspec S1) \<and> f2\<in>(Rep_fst_uspec S2)};
...
in Abs_fst_uspec Comp_set Cin Cout"

(* Parallel Composition *)
definition uspec_parcompwell :: "('m,'n) ufun uspec \<Rightarrow> ('m,'n) ufun uspec \<Rightarrow> bool" where
"uspec_parcompwell S1 S2 \<equiv> \<forall> f1 \<in> (Rep_fst_uspec S1). \<forall> f2 \<in> (Rep_fst_uspec S2). parcomp_well f1 f2"  
...
let Comp_set = {ufParComp f1 f2 | f1 f2.  f1\<in>(Rep_fst_uspec S1) \<and> f2\<in>(Rep_fst_uspec S2)};
...
"uspec_sercompwell S1 S2 \<equiv> \<forall> f1 \<in> (Rep_fst_uspec S1). \<forall> f2 \<in> (Rep_fst_uspec S2). sercomp_well f1 f2"
...
let Comp_set = {ufSerComp f1 f2 | f1 f2.  f1\<in>(Rep_fst_uspec S1) \<and> f2\<in>(Rep_fst_uspec S2)};
...
let Comp_set = {ufFeedbackComp f1 | f1.  f1\<in>(Rep_fst_uspec S1)};
...
lemma uspec_compwell2ufunclcompwell: assumes "uspec_compwell S1 S2"
  shows "\<forall> f1 \<in> Rep_fst_uspec S1. \<forall> f2 \<in> Rep_fst_uspec S2. comp_well f1 f2"
  using assms uspec_compwell_def by blast

lemma uspec_comp_well[simp]: assumes "uspec_compwell S1 S2"
  shows "uspecWell {ufComp f1 f2 |(f1::('m,'m) ufun) (f2::('m,'m) ufun). f1 \<in> Rep_fst_uspec S1 \<and> f2 \<in> Rep_fst_uspec S2}
              (Discr (uspecCompDom S1 S2)) (Discr (uspecRanUnion S1 S2))"
  apply (rule uspec_wellI) 
  apply (smt CollectD assms comp_well_def inf.idem sup_idem ufCompI_def ufclDom_ufun_def 
    ufclRan_ufun_def ufcomp_dom uspecInter_general_well uspecWell.simps uspec_allDom 
    uspec_compwell2ufunclcompwell uspecset_insert)
  using assms ufclDom_ufun_def ufclRan_ufun_def ufcomp_ran
  by (smt CollectD comp_well_def ufCompO_def uspec_allRan uspec_compwell2ufunclcompwell uspecset_insert)

(*   *)
lemma uspec_comp_commu: assumes "uspec_compwell S1 S2"
  shows "(S1 \<Otimes> S2) = (S2 \<Otimes> S1)"          
proof - 
  have "{ufComp f1 f2 |(f1::('a,'a) ufun) (f2::('a,'a) ufun). f1 \<in> Rep_fst_uspec S1 \<and> f2 \<in> Rep_fst_uspec S2} = 
             {ufComp f1 f2 |(f1::('a,'a) ufun) (f2::('a,'a) ufun). f1 \<in> Rep_fst_uspec S2 \<and> f2 \<in> Rep_fst_uspec S1}" (is "?L = ?R")
    apply auto
...
shows "{f1 \<otimes> f2 |(f1::('a,'a) ufun) (f2::('a,'a) ufun). f1 \<in> Rep_fst_uspec S1 \<and> f2 \<in> Rep_fst_uspec S2} 
          = Rep_fst_uspec (S1 \<Otimes> S2)"
  apply (simp add: uspecComp_def)
  by (metis (mono_tags, lifting) assms uspec_abs_set uspec_comp_well uspecset_insert)

lemma uspec_comp_ele_ex: assumes "uspec_compwell S1 S2"
and "uspecIsConsistent (S1 \<Otimes> S2)"
shows "\<forall> f \<in> Rep_fst_uspec (S1 \<Otimes> S2). \<exists> f1 \<in> Rep_fst_uspec S1. \<exists> f2 \<in> Rep_fst_uspec S2. f = (f1 \<otimes> f2)"
  using assms uspec_comp_rep by fastforce

lemma uspec_comp_not_empty:  assumes "uspec_compwell S1 S2"
"f1 \<in> Rep_fst_uspec S1" and f2_def: "f2 \<in> Rep_fst_uspec S2"
shows "(f1 \<otimes> f2) \<in> Rep_fst_uspec (S1 \<Otimes> S2)"
  using assms(1) assms(2) f2_def mem_Collect_eq uspec_comp_rep by fastforce

lemma uspec_comp_consistent2: assumes "uspecIsConsistent (S1 \<Otimes> S2)"
  shows "uspecIsConsistent S1 \<and> uspecIsConsistent S2"
  by (smt Collect_empty_eq assms empty_iff uspecIsConsistent_def uspec_comp_rep uspec_compwell_def uspecset_insert)
...
lemma uspec_parcompwell2ufunclparcompwell: assumes "uspec_parcompwell S1 S2"
  shows "\<forall> f1 \<in> Rep_fst_uspec S1. \<forall> f2 \<in> Rep_fst_uspec S2. parcomp_well f1 f2"
  using assms uspec_parcompwell_def 
  by blast

(*   *)
lemma uspec_parcomp_well[simp]: assumes "uspec_parcompwell S1 S2"
  shows "uspecWell {ufParComp f1 f2 |(f1::('a,'b) ufun) (f2::('a,'b) ufun). f1 \<in> Rep_fst_uspec S1 \<and> f2 \<in> Rep_fst_uspec S2}
              (Discr (uspecDomUnion S1 S2)) (Discr (uspecRanUnion S1 S2))"
  apply (rule uspec_wellI) 
  using ufclDom_ufun_def
  apply (smt CollectD assms ufParComp_dom uspec_allDom uspec_parcompwell2ufunclparcompwell uspecset_insert)
  using ufclDom_ufun_def
  by (smt CollectD assms ufParComp_ran ufclRan_ufun_def uspec_allRan uspec_parcompwell2ufunclparcompwell uspecset_insert)

(*   *)
lemma uspec_parcomp_rep[simp]: assumes "uspec_parcompwell S1 S2"
shows "{ufParComp f1 f2 |(f1::('a,'b) ufun) (f2::('a,'b) ufun). f1 \<in> Rep_fst_uspec S1 \<and> f2 \<in> Rep_fst_uspec S2} 
          = Rep_fst_uspec ((S1 :: ('a,'b) ufun uspec) \<parallel> S2)"
  by (metis (mono_tags, lifting) assms uspecParComp_def uspec_abs_set uspec_parcomp_well uspecset_insert)

(*   *)
lemma uspec_parcomp_not_empty:  assumes "uspec_parcompwell S1 S2"
"f1 \<in> Rep_fst_uspec S1" and f2_def: "f2 \<in> Rep_fst_uspec S2"
shows "(ufParComp f1 f2) \<in> Rep_fst_uspec (S1 \<parallel> S2)"
...
shows "uspecIsConsistent (S1 \<parallel> S2)"
  using assms(1) assms(2) assms(3) emptyE some_in_eq uspecIsConsistent_def uspec_parcomp_not_empty
  by (metis (mono_tags) uspecSet.rep_eq)

(*   *)
lemma uspec_parcomp_consistent2: assumes "uspec_parcompwell S1 S2" and "uspecIsConsistent (S1 \<parallel> S2)"
  shows "uspecIsConsistent S1 \<and> uspecIsConsistent S2"
proof (rule)
  have f1: "{ufParComp a aa |a aa. a \<in> Rep_fst_uspec S1 \<and> aa \<in> Rep_fst_uspec S2} \<noteq> {}"
    using assms(1) assms(2) uspecIsConsistent_def
    by (simp add: uspecIsConsistent_def uspecset_insert)   
  then have "Rep_fst_uspec S1 \<noteq> {}"
    by blast
  then show "uspecIsConsistent S1"
    unfolding uspecIsConsistent_def
    by (simp add: uspecset_insert)
  have "Rep_fst_uspec S2 \<noteq> {}"
    using f1 by blast
...
lemma uspec_parcomp_h1: assumes "uspec_parcompwell S1 S2"
  shows "\<forall> f1 \<in> Rep_fst_uspec S1. \<forall> f2 \<in> Rep_fst_uspec S2. (ufParComp f1 f2) \<in> Rep_fst_uspec (S1 \<parallel> S2)"
...
shows "\<forall> f \<in> Rep_fst_uspec (S1 \<parallel> S2). \<exists> f1 \<in> Rep_fst_uspec S1. \<exists> f2 \<in> Rep_fst_uspec S2. f = (ufParComp f1 f2)"
...
proof -
  have f1: "{f1 \<parallel> f2 |f1 f2. f1 \<in> Rep_fst_uspec S1 \<and> f2 \<in> Rep_fst_uspec S2} = 
                {f1 \<parallel> f2 |f1 f2. f1 \<in> Rep_fst_uspec S2 \<and> f2 \<in> Rep_fst_uspec S1}" (is "?L = ?R")
    apply auto
...
proof -
  ...
  have f2: "uspecSet\<cdot>((S1 \<parallel> S2) \<parallel> S3) = 
      {f1 \<parallel> f2 |f1 f2. f1 \<in> Rep_fst_uspec (S1 \<parallel> S2) \<and> f2 \<in> Rep_fst_uspec S3}"
    apply (simp add: uspecset_insert)
    apply (subst uspec_parcomp_rep)
    using assms(1) assms(2) assms(3) uspec_parcomp_ele_ex uspec_parcompwell2ufunclparcompwell uspec_parcompwell_def 
    using f0 apply blast
    by simp
  have f3: "uspecSet\<cdot>(S1 \<parallel> (S2 \<parallel> S3)) = 
      {f1 \<parallel> f2 |f1 f2. f1 \<in> Rep_fst_uspec S1 \<and> f2 \<in> Rep_fst_uspec (S2 \<parallel> S3)}"
    apply (simp add: uspecset_insert)
    by (simp add: f1)
  have f10: "{f1 \<parallel> f2 |f1 f2. f1 \<in> Rep_fst_uspec (S1 \<parallel> S2) \<and> f2 \<in> Rep_fst_uspec S3} =
               {f1 \<parallel> f2 |f1 f2. f1 \<in> Rep_fst_uspec S1 \<and> f2 \<in> Rep_fst_uspec (S2 \<parallel> S3)}"
  proof(auto)
    show "\<And>f1 f2.
       f1 \<in> Rep_fst_uspec (S1 \<parallel> S2) \<Longrightarrow>
       f2 \<in> Rep_fst_uspec S3 \<Longrightarrow> 
       \<exists>f1a f2a. (f1 \<parallel> f2) = (f1a \<parallel> f2a) \<and> f1a \<in> Rep_fst_uspec S1 \<and> f2a \<in> Rep_fst_uspec (S2 \<parallel> S3)"
    proof -
      fix f1::"'a\<Rrightarrow> 'b" and f2::"'a\<Rrightarrow> 'b"
      assume f1_def: "f1 \<in> Rep_fst_uspec (S1 \<parallel> S2)" and f2_def: "f2 \<in> Rep_fst_uspec S3"
      obtain f3 f4 where f3_def: "f3 \<in> Rep_fst_uspec S1" and f4_def: "f4 \<in> Rep_fst_uspec S2" and f1_eq_f3_f4: "f1 = (f3 \<parallel> f4)"
        by (metis assms(1) f1_def uspec_parcomp_ele_ex)
      then show "\<exists>f1a f2a. (f1 \<parallel> f2) = (f1a \<parallel> f2a) \<and> f1a \<in> Rep_fst_uspec S1 \<and> f2a \<in> Rep_fst_uspec (S2 \<parallel> S3)"
        using f1_def f2_def f3_def f4_def
        by (meson assms ufParComp_asso uspec_parcomp_h2 uspec_parcomp_not_empty)
    qed
  next 
    show "\<And>f1 f2.
       f1 \<in> Rep_fst_uspec S1 \<Longrightarrow>
       f2 \<in> Rep_fst_uspec (S2 \<parallel> S3) \<Longrightarrow> 
       \<exists>f1a f2a. (f1 \<parallel> f2) = (f1a \<parallel> f2a) \<and> f1a \<in> Rep_fst_uspec (S1 \<parallel> S2) \<and> f2a \<in> Rep_fst_uspec S3"
     proof -
      fix f1::"'a\<Rrightarrow> 'b" and f2::"'a\<Rrightarrow> 'b"
      assume f1_def: "f1 \<in> Rep_fst_uspec S1" and f2_def: "f2 \<in> Rep_fst_uspec (S2 \<parallel> S3)"
      obtain f3 f4 where f3_def: "f3 \<in> Rep_fst_uspec S2" and f4_def: "f4 \<in> Rep_fst_uspec S3" and f2_eq_f3_f4: "f2 = (f3 \<parallel> f4)"
        by (metis assms(3) f2_def uspec_parcomp_ele_ex)
      then show "\<exists>f1a f2a. (f1 \<parallel> f2) = (f1a \<parallel> f2a) \<and> f1a \<in> Rep_fst_uspec (S1 \<parallel> S2) \<and> f2a \<in> Rep_fst_uspec S3"
        using f1_def f2_def f3_def f4_def
        by (smt assms ufParComp_asso uspec_parcomp_h2 uspec_parcomp_not_empty)
    qed
  qed
  have f11: "uspecSet\<cdot>(S1 \<parallel> S2 \<parallel> S3) = uspecSet\<cdot>(S1 \<parallel> (S2 \<parallel> S3))"
    apply (subst f2)
    apply (subst f3)
    by (simp add: f10)
  show ?thesis
...
and "uspec_parcompwell S2 S3" 
shows "uspecWell  {f1 \<parallel> f2 |f1 f2. f1 \<in> Rep_fst_uspec S1 \<and> f2 \<in> Rep_fst_uspec (S2 \<parallel> S3)}
...
lemma uspec_sercomp_well[simp]: assumes "uspec_sercompwell S1 S2"
  shows "uspecWell {f1 \<circ> f2 |f1 f2. f1 \<in> Rep_fst_uspec S1 \<and> f2 \<in> Rep_fst_uspec S2}
         (Discr (uspecDom\<cdot>S1)) (Discr (uspecRan\<cdot>S2))"
  apply (rule uspec_wellI)
  using ufclDom_ufun_def
  apply (smt CollectD assms ufSerComp_dom uspec_allDom uspec_sercompwell_def uspecset_insert)
  using ufclRan_ufun_def 
  by (smt CollectD assms ufSerComp_ran uspec_allRan uspec_sercompwell_def uspecset_insert)

lemma uspec_sercompwell2ufunclsercompwell: assumes "uspec_sercompwell S1 S2"
  shows "\<forall> f1 \<in> Rep_fst_uspec S1. \<forall> f2 \<in> Rep_fst_uspec S2. sercomp_well f1 f2"
  using assms uspec_sercompwell_def by blast

(*   *)
lemma uspec_sercomp_rep[simp]: assumes "uspec_sercompwell S1 S2"
shows "{f1 \<circ> f2 |f1 f2. f1 \<in> Rep_fst_uspec S1 \<and> f2 \<in> Rep_fst_uspec S2} 
          = Rep_fst_uspec (S1 \<circle> S2)"
  apply (simp add: uspecSerComp_def)
  by (smt Collect_cong assms prod.collapse prod.inject rep_abs_uspec uspec_sercomp_well)

(*   *)
lemma uspec_sercomp_not_empty:  assumes "uspec_sercompwell S1 S2"
"f1 \<in> Rep_fst_uspec S1" and f2_def: "f2 \<in> Rep_fst_uspec S2"
shows "(f1 \<circ> f2) \<in> Rep_fst_uspec (S1 \<circle> S2)"
...
shows "uspecIsConsistent (S1 \<circle> S2)"
  apply (insert assms)
  unfolding uspecIsConsistent_def
  using uspec_sercomp_not_empty
  by (metis assms(1) empty_iff neq_emptyD uspecset_insert)
...
lemma uspec_sercomp_consistent2: assumes "uspecIsConsistent (S1 \<circle> S2)"
  ...
  apply (insert assms)
  unfolding uspecIsConsistent_def
  using uspec_sercomp_rep uspec_sercompwell_def
  by (smt Collect_empty_eq empty_iff uspecset_insert)
...
and "f \<in> Rep_fst_uspec (S1 \<circle> S2)"
shows "\<exists> f1 \<in> Rep_fst_uspec S1. \<exists> f2 \<in> Rep_fst_uspec S2. f = (f1 \<circ> f2)"
  using assms(1) assms(2) uspec_sercomp_rep by fastforce

lemma uspec_sercomp_h1: assumes "uspec_sercompwell S1 S2"
  shows "\<forall> f1 \<in> Rep_fst_uspec S1. \<forall> f2 \<in> Rep_fst_uspec S2. (f1 \<circ> f2) \<in> Rep_fst_uspec (S1 \<circle> S2)"
...
proof -
  ...
  proof (simp add: uspec_sercompwell_def, rule, rule)
    ...
    assume a1: "f1 \<in> Rep_fst_uspec (S1 \<circle> S2)"
    assume a2: "f2 \<in> Rep_fst_uspec S3"
    obtain f3 f4 where f3_def: "(f3 ::('a,'b) ufun) \<in> Rep_fst_uspec S1" and f4_def: "(f4 ::('b,'c) ufun) \<in> Rep_fst_uspec S2" 
    ...
    have f3: "ufclDom\<cdot>f3 \<inter> ufclRan\<cdot>f4 = {}"
      by (metis assms(3) f3_def f4_def uspec_allDom uspec_allRan uspecset_insert)
    have f4: "ufclDom\<cdot>f4 \<inter> ufclRan\<cdot>f2 = {}"
      by (metis a2 assms(4) f4_def uspec_allDom uspec_allRan uspecset_insert)
  ...
  proof (simp add: uspec_sercompwell_def, rule, rule)
    ...
    assume a1: "f1 \<in> Rep_fst_uspec S1"
    assume a2: "f2 \<in> Rep_fst_uspec (S2 \<circle> S3)"
    obtain f3 f4 where f3_def: "(f3 :: ('b,'c) ufun) \<in> Rep_fst_uspec S2" and f4_def: "(f4 :: ('c,'d) ufun) \<in> Rep_fst_uspec S3" 
    ...
    have f3: "ufclDom\<cdot>f1 \<inter> ufclRan\<cdot>f3 = {}"
      by (metis a1 assms(3) f3_def uspec_allDom uspec_allRan uspecset_insert)
    have f4: "ufclDom\<cdot>f3 \<inter> ufclRan\<cdot>f4 = {}"
      by (metis assms(4) f3_def f4_def uspec_allDom uspec_allRan uspecset_insert)
    show "sercomp_well f1 f2"
  ...
  qed
  have f2: "uspecSet\<cdot>(S1 \<circle> S2 \<circle> S3) = 
      {f1 \<circ> f2 |f1 f2. f1 \<in> Rep_fst_uspec (S1 \<circle> S2) \<and> f2 \<in> Rep_fst_uspec S3}"
    apply (simp add: uspecset_insert)
    apply (subst uspec_sercomp_rep)
     apply (simp add: f0)
    by simp
  have f3: "uspecSet\<cdot>(S1 \<circle> (S2 \<circle> S3)) = 
      {f1 \<circ> f2 |f1 f2. f1 \<in> Rep_fst_uspec S1 \<and> f2 \<in> Rep_fst_uspec (S2 \<circle> S3)}"
    apply (simp add: uspecset_insert)
    apply (subst uspec_sercomp_rep)
    apply (simp add: f1)
    by simp
  have f10: "{ ufSerComp f1 f2 |(f1::('a,'c) ufun) (f2::('c,'d) ufun). f1 \<in> (Rep_fst_uspec (S1 \<circle> S2)) \<and> (f2 \<in> Rep_fst_uspec S3)} =
              { ufSerComp f1 f2 |(f1::('a,'b) ufun) (f2::('b,'d) ufun). f1 \<in> (Rep_fst_uspec S1) \<and> f2 \<in> (Rep_fst_uspec (S2 \<circle> S3))}"
  (*apply (rule subset_antisym)
  apply (rule subsetI)*)
  proof(auto)
    show "\<And>f1 f2.
       f1 \<in> Rep_fst_uspec (S1 \<circle> S2) \<Longrightarrow>
       f2 \<in> Rep_fst_uspec S3 \<Longrightarrow>
       \<exists>f1a f2a. (f1 \<circ> f2) = (f1a \<circ> f2a) \<and> f1a \<in> Rep_fst_uspec S1 \<and> f2a \<in> Rep_fst_uspec (S2 \<circle> S3)"
    proof -
      fix f1::"'a\<Rrightarrow> 'c" and f2::"'c\<Rrightarrow> 'd"
      assume f1_def: "f1 \<in> Rep_fst_uspec (S1 \<circle> S2)" and f2_def: "f2 \<in> Rep_fst_uspec S3"
      obtain f3 f4 where f3_def: "f3 \<in> Rep_fst_uspec S1" and f4_def: "f4 \<in> Rep_fst_uspec S2" and f1_eq_f3_f4: "f1 = (f3 \<circ> f4)"
      ...
      then show "\<exists>f1a f2a. (f1 \<circ> f2) = (f1a \<circ> f2a) \<and> f1a \<in> Rep_fst_uspec S1 \<and> f2a \<in> Rep_fst_uspec (S2 \<circle> S3)"
  ...
  next
    show "\<And>f1 f2. 
        f1 \<in> Rep_fst_uspec S1 \<Longrightarrow> 
        f2 \<in> Rep_fst_uspec (S2 \<circle> S3) \<Longrightarrow> 
        \<exists>f1a f2a. (f1 \<circ> f2) = (f1a \<circ> f2a) \<and> f1a \<in> Rep_fst_uspec (S1 \<circle> S2) \<and> f2a \<in> Rep_fst_uspec S3"
    proof -
      fix f1::"'a\<Rrightarrow> 'b" and f2::"'b\<Rrightarrow> 'd"
      assume f1_def: "f1 \<in> Rep_fst_uspec S1" and f2_def: "f2 \<in> Rep_fst_uspec (S2 \<circle> S3)"
      obtain f3 f4 where f3_def: "f3 \<in> Rep_fst_uspec S2" and f4_def: "f4 \<in> Rep_fst_uspec S3" and f2_eq_f3_f4: "f2 = (f3 \<circ> f4)"
      ...
      then show "\<exists>f1a f2a. (f1 \<circ> f2) = (f1a \<circ> f2a) \<and> f1a \<in> Rep_fst_uspec (S1 \<circle> S2) \<and> f2a \<in> Rep_fst_uspec S3"
        using assms(1) f1_def f2_def f3_def f4_def 
        by (meson assms ufSerComp_asso uspec_sercomp_h2 uspec_sercomp_not_empty)
    qed
  qed
  have f11: "uspecSet\<cdot>(S1 \<circle> S2 \<circle> S3) = uspecSet\<cdot>(S1 \<circle> (S2 \<circle> S3))"
...
lemma uspec_feedbackcomp_well: "uspecWell {ufFeedbackComp f1 | f1.  f1\<in>(Rep_fst_uspec S1)}
  (Discr (uspecDom\<cdot>S1 - uspecRan\<cdot>S1)) (Discr (uspecRan\<cdot>S1))"
  apply (rule uspec_wellI)
  using ufclDom_ufun_def ufFeedbackComp_dom
  apply (smt mem_Collect_eq ufclRan_ufun_def uspec_allDom uspec_allRan uspecset_insert)
  using ufclRan_ufun_def ufFeedbackComp_ran
  by (smt mem_Collect_eq  uspec_allDom uspec_allRan uspecset_insert)

lemma uspec_feedbackcomp_insert: "uspecFeedbackComp S1 = 
  Abs_fst_uspec  {ufFeedbackComp f1 | f1.  f1\<in>(Rep_fst_uspec S1)}
...
lemma uspec_feedbackcomp_f_ex: assumes "f \<in> Rep_fst_uspec (uspecFeedbackComp S)"
  shows "\<exists> f1 \<in> Rep_fst_uspec S. f = ufFeedbackComp f1"
proof -
  have "Rep_uspec (Abs_fst_uspec {ufFeedbackComp a |a. a \<in> Rep_fst_uspec S} (uspecDom\<cdot>S - uspecRan\<cdot>S) (uspecRan\<cdot>S)) = ({ufFeedbackComp a |a. a \<in> Rep_fst_uspec S}, Discr (uspecDom\<cdot>S - uspecRan\<cdot>S), Discr (uspecRan\<cdot>S))"
    using rep_abs_uspec uspec_feedbackcomp_well by blast
  then have "{ufFeedbackComp a |a. a \<in> Rep_fst_uspec S} = Rep_fst_uspec (uspecFeedbackComp S)"
    by (simp add: uspec_feedbackcomp_insert)
  then have "\<exists>a. f = (ufFeedbackComp a) \<and> a \<in> Rep_fst_uspec S"
