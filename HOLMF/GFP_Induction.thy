...
"longIterate C f = gfp (Pow C) (\<lambda> (S::'a set).  insert (div_bot C) (f `S))"
...
proof -
  have "Pow C \<in> DIV"
    by (simp add: DIV_set_def assms(1))
  then show ?thesis
    by (metis (no_types) PowD assms(1) assms(2) gfp_div longIterate_def longiterate_good longiterate_mono)
qed
...
lemma long_iterate_below_fix: assumes "monofun f" and "C\<in>DIV" and "goodFormed C f"
  ...
  apply(rule gfp_smaller)
  apply(auto simp add: assms DIV_set_def less_set_def)
  oops
