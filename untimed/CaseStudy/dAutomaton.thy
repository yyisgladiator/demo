...
"da_helper f s \<equiv> \<Lambda> h. (\<lambda> e. spfRt\<cdot>(spfConc (snd (f (s,e)))\<cdot>(h (fst (f (s,e))))))"
...
"da_H automat = spfConc (daInitialOutput automat)\<cdot>(da_h automat (daInitialState automat))"
...
lemma da_h_final:
  ...
  spfConc (daNextOutput automat s ((inv convDiscrUp)(sbHdElem\<cdot>sb)))\<cdot>(spfRt\<cdot>(da_h automat (daNextState automat s ((inv convDiscrUp)(sbHdElem\<cdot>sb))))) \<rightleftharpoons>sb"
  apply(subst da_h_step, simp_all add: assms)
  by (simp add: assms(1) daNextOutput_def daNextState_def da_helper_def spfRt_spfConc)
