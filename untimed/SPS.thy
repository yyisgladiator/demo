...
"spsConcOut sb = Abs_cfun (uspecImage (Rep_cfun (spfConc sb)))"

definition spsRtIn:: "'m SPS \<rightarrow> 'm SPS" where
"spsRtIn = Abs_cfun (uspecImage (Rep_cfun spfRt))"
...
lemma spsconcout_cont: 
  ...
  shows"cont (uspecImage (Rep_cfun (spfConc sb)))"
...
lemma spsconcout_insert: 
  assumes "\<And>c. c\<in>ubDom\<cdot>sb \<Longrightarrow> # (sb . c) < \<infinity>"
  shows"spsConcOut sb\<cdot>sps =  (uspecImage (Rep_cfun (spfConc sb)) sps)"
...
lemma spsrtin_cont: "cont (uspecImage (Rep_cfun spfRt))"
  apply(rule uspecimage_inj_cont)
   apply (simp add: spfRt_inj)
  by (simp add: ufclDom_ufun_def ufclRan_ufun_def)

lemma spsrtin_insert: "spsRtIn\<cdot>sps = uspecImage (Rep_cfun spfRt) sps"
