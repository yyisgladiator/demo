...
theory Receiver
imports ReceiverAutomaton Components "../../../UBundle_Induction" HOLCF.Adm
...
lemma createobundle_ubdom: "ubDom\<cdot>(createOBundle a)= {\<C> ''o''}"
  by (simp add: ubDom_def createOBundle.rep_eq)

lemma createaroutput_createooutput_ubclunion_ubdom:
  "ubDom\<cdot>((createArBundle a) \<uplus> (createOBundle b)) = {\<C> ''ar'', \<C> ''o''}"
  by (metis (no_types, lifting) createarbundle_ubdom createobundle_ubdom insert_is_Un 
      ubclDom_ubundle_def ubclunion_ubcldom)

lemma createaroutput_tsynbnullo_ubclunion_ubdom: 
  "ubDom\<cdot>((createArBundle a) \<uplus> (tsynbNull (\<C> ''o''))) = {\<C> ''ar'', \<C> ''o''}"
  by (metis (no_types, lifting) createarbundle_ubdom insert_is_Un tsynbnull_ubdom 
      ubclDom_ubundle_def ubclunion_ubcldom)
...
lemma receiverautomaton_dainitialoutput:
...
lemma receiverautomaton_da_h_ubdom:
  ...
  by (simp add: assms da_h_ubdom receiverautomaton_dadom receiverautomaton_daran)
...
lemma receiverautomaton_h_step_rf_null:
  ...
  by (simp add: assms da_h_final receiverautomaton_dadom receiverautomaton_daran da_h_ubdom
      ReceiverAutomaton.rep_eq daNextOutput_def daNextState_def daTransition_def usclConc_stream_def
      ubclUnion_ubundle_def receiverautomaton_da_h_ubdom)
...
lemma receiverautomaton_h_step_rf_true:
  ...
  by (simp add: assms da_h_final receiverautomaton_dadom receiverautomaton_daran da_h_ubdom
      ReceiverAutomaton.rep_eq daNextOutput_def daNextState_def daTransition_def usclConc_stream_def
      ubclUnion_ubundle_def receiverautomaton_da_h_ubdom createarbundle_ubdom)
...
lemma receiverautomaton_h_step_rt_true: 
  ...
  by (simp add: assms da_h_final receiverautomaton_dadom receiverautomaton_daran da_h_ubdom
      ReceiverAutomaton.rep_eq daNextOutput_def daNextState_def daTransition_def usclConc_stream_def
      ubclUnion_ubundle_def receiverautomaton_da_h_ubdom createarbundle_ubdom createobundle_ubdom)
...
lemma receiverautomaton_H_step:
  ...
  shows "da_H ReceiverAutomaton \<rightleftharpoons> sb
           = ubConc ((tsynbNull (\<C> ''ar'')) \<uplus> (tsynbNull (\<C> ''o'')))
               \<cdot>(da_h ReceiverAutomaton (ReceiverState Rt) \<rightleftharpoons> sb)"
  by (simp add: assms da_H_def da_h_ubdom receiverautomaton_dadom receiverautomaton_daran
      daInitialState_def daInitialOutput_def ReceiverAutomaton.rep_eq receiverautomaton_da_h_ubdom 
      tsynbnullar_tsynbnullo_ubclunion_ubdom)
...
lemma receiverspf_strict:
  ...
  by (simp add: ReceiverSPF_def da_H_bottom receiverautomaton_dadom receiverautomaton_daran 
                receiverautomaton_dainitialoutput tsynbnullar_tsynbnullo_ubclunion_ubdom)
...
(* further lemmata to avoid different assumption *)
...
lemma recspf_ubconc_null:
  ...
  apply (rule ub_eq)
  apply (simp add: assms recspf_insert tsynbrec_ubdom ubclUnion_ubundle_def tsynbNull.rep_eq  
                   insert_commute ) 
  apply (rename_tac x)
  apply (case_tac "x = \<C> ''o''")
  by (simp add: assms recspf_insert tsynbrec_ubdom ubclUnion_ubundle_def tsynbNull.rep_eq 
      tsynbrec_getch_ar tsynbrec_getch_o usclConc_stream_def bool2abp_def nat2abp_def 
      abp2natbool_def tsynmap_sconc_null tsynrec_sconc_null tsynprojsnd_sconc_null)+
...
lemma recspf_true_ubconc_true:
  ...
  by (simp add: assms recspf_insert tsynbrec_ubdom ubclUnion_ubundle_def createarbundle_ubdom 
      createobundle_ubdom tsynbrec_getch_o tsynbrec_getch_ar usclConc_stream_def 
...
lemma recspf_true_ubconc_false:
  ...
  apply (case_tac "x = \<C> ''o''")
  by (simp add: assms recspf_insert tsynbrec_ubdom ubclUnion_ubundle_def createarbundle_ubdom 
      tsynbrec_getch_o tsynbrec_getch_ar usclConc_stream_def createarbundle_ubgetch nat2abp_def 
      bool2abp_def abp2natbool_def tsynmap_sconc_msg tsynmap_sconc_null pair_invpair_inv 
      tsynrec_sconc_msg_f_alt tsynprojsnd_insert)+
...
lemma recspf_false_ubconc_true:
  ...
  apply (case_tac "x = \<C> ''o''")
  by (simp add: assms recspf_insert tsynbrec_ubdom ubclUnion_ubundle_def createarbundle_ubdom 
      tsynbrec_getch_o tsynbrec_getch_ar usclConc_stream_def createarbundle_ubgetch 
      nat2abp_def bool2abp_def abp2natbool_def tsynmap_sconc_msg tsynmap_sconc_null 
      pair_invpair_inv tsynrec_sconc_msg_f_alt tsynprojsnd_insert)+
...
definition statemap:: "ReceiverSubstate \<Rightarrow> bool" where
 "statemap a \<equiv> if (a = Rf) then False else True"
...
proof -
    have "\<And>b. da_h ReceiverAutomaton (ReceiverState b) \<rightleftharpoons> sb = RecSPF (statemap b) \<rightleftharpoons> sb"
    proof (induction sb  arbitrary:b rule:ind_ub)
      case 1
      then show ?case 
         proof (rule admI)
            fix Y :: "nat \<Rightarrow> abpMessage tsyn stream\<^sup>\<Omega>"
            assume chain_Y: "chain Y"
            assume adm_hyp: "\<forall>(i::nat) x. da_h ReceiverAutomaton (ReceiverState x) \<rightleftharpoons> Y i 
                                        = RecSPF (statemap x) \<rightleftharpoons> Y i"
            have "\<forall> x.(\<Squnion>n. da_h ReceiverAutomaton (ReceiverState x) \<rightleftharpoons> Y n) 
                     = (\<Squnion>n. RecSPF (statemap x) \<rightleftharpoons> Y n)"
              using adm_hyp by fastforce
            then show "\<forall> x. da_h ReceiverAutomaton (ReceiverState x) \<rightleftharpoons> (\<Squnion>i::nat. Y i) 
                    = RecSPF (statemap x) \<rightleftharpoons> (\<Squnion>i::nat. Y i)"
      ...
      then show ?case
          ...
          next
            ...
            from ctype_m obtain a where m_def: "m = Pair_nat_bool a"
              by auto
            have " snd a \<Longrightarrow> b=Rf \<Longrightarrow>
                (da_h ReceiverAutomaton (ReceiverState b) \<rightleftharpoons> ubConc (createBundle (\<M> m) (\<C> ''dr''))\<cdot>ub) 
                 = (RecSPF (statemap b) \<rightleftharpoons> ubConc (createBundle (\<M> m) (\<C> ''dr''))\<cdot>ub)"
              by (simp add: "3.IH" assms m_def receiverautomaton_h_step_rf_true receiverspf_ufdom recspf_false_ubconc_true statemap_def)
            moreover have "\<not> snd a \<Longrightarrow> b=Rf \<Longrightarrow>
                (da_h ReceiverAutomaton (ReceiverState b) \<rightleftharpoons> ubConc (createBundle (\<M> m) (\<C> ''dr''))\<cdot>ub)
                 = (RecSPF (statemap b) \<rightleftharpoons> ubConc (createBundle (\<M> m) (\<C> ''dr''))\<cdot>ub)"
              by (simp add: "3.IH" assms eq_conc_rf_false m_def receiverspf_ufdom statemap_def)
            moreover have "snd a \<Longrightarrow> b=Rt \<Longrightarrow>
                (da_h ReceiverAutomaton (ReceiverState b) \<rightleftharpoons> ubConc (createBundle (\<M> m) (\<C> ''dr''))\<cdot>ub)
                 = (RecSPF (statemap b) \<rightleftharpoons> ubConc (createBundle (\<M> m) (\<C> ''dr''))\<cdot>ub)"
              by (simp add: "3.IH" assms m_def receiverautomaton_h_step_rt_true receiverspf_ufdom recspf_true_ubconc_true statemap_def)
            moreover have "\<not> snd a \<Longrightarrow> b=Rt \<Longrightarrow>
                (da_h ReceiverAutomaton (ReceiverState b) \<rightleftharpoons> ubConc (createBundle (\<M> m) (\<C> ''dr''))\<cdot>ub) 
                 = (RecSPF (statemap b) \<rightleftharpoons> ubConc (createBundle (\<M> m) (\<C> ''dr''))\<cdot>ub)"
              by (simp add: "3.IH" assms eq_conc_rt_false m_def receiverspf_ufdom statemap_def)
            ultimately show 
              "da_h ReceiverAutomaton (ReceiverState b) \<rightleftharpoons>
              ubConc (createBundle (\<M> m) (\<C> ''dr''))\<cdot>ub =
              RecSPF (statemap b) \<rightleftharpoons> ubConc (createBundle (\<M> m) (\<C> ''dr''))\<cdot>ub"
              using ReceiverSubstate.exhaust by blast
          next
      case null
      show "\<And>b. da_h ReceiverAutomaton (ReceiverState b) \<rightleftharpoons> ubConc (tsynbNull (\<C> ''dr''))\<cdot>ub =
            RecSPF (statemap b) \<rightleftharpoons> ubConc (tsynbNull (\<C> ''dr''))\<cdot>ub"
        by (metis (full_types) "3.IH" ReceiverSubstate.exhaust assms receiverautomaton_h_step_rf_null receiverautomaton_h_step_rt_null receiverspf_ufdom recspf_ubconc_null)
    qed
  qed
  then show "ReceiverSPF \<rightleftharpoons> sb = ubConc (tsynbNull (\<C> ''ar'') \<uplus> tsynbNull (\<C> ''o''))\<cdot>(RecSPF True \<rightleftharpoons> sb)"
    apply (simp add: ReceiverSPF_def)
    apply (subst receiverautomaton_H_step)
    apply (simp add: assms receiverspf_ufdom)
    by (simp add: statemap_def)
  qed
