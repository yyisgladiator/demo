...
lemma tsynzip_sdom_msg2: "sdom\<cdot>(tsynZip\<cdot>as\<cdot>bs) \<subseteq> sdom\<cdot>(tsynZip\<cdot>(\<up>(\<M> a) \<bullet> as)\<cdot>(\<up>b \<bullet> bs))"
  by (simp add: subset_insertI tsynzip_sconc_msg)

lemma tsynzip_sdom_null: "sdom\<cdot>(tsynZip\<cdot>as\<cdot>bs) \<subseteq> sdom\<cdot>(tsynZip\<cdot>(\<up>- \<bullet> as)\<cdot>bs) \<union> {null} "
