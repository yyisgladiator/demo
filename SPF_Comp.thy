...
section \<open>iter_sbfix2\<close>
...
section \<open>lub_iter_sbfix2\<close>
...
section \<open>if_lub_iter_sbfix2\<close>   
  ...
  have "\<And> x. \<And> y. x \<sqsubseteq> y \<Longrightarrow> ((\<lambda> x. (P x) \<leadsto> (\<Squnion>i.(iter_sbfix2 F i cs) x)) x) 
    ...
    have f5: "\<And>p. sbDom\<cdot>(F x\<cdot>(cs^\<bottom>)) \<noteq> cs \<or> 
      ...
      ultimately have "P x \<and> P y \<longrightarrow> (P x)\<leadsto>\<Squnion>n. iter_sbfix2 F n cs x 
        ...
        using assms(2) by auto
      then have "sbDom\<cdot>y = sbDom\<cdot>x \<and> P x \<longrightarrow> (P x)\<leadsto>\<Squnion>n. iter_sbfix2 F n cs x 
                                          \<sqsubseteq> (P y)\<leadsto>\<Squnion>n. iter_sbfix2 F n cs y"
        by (meson assms(3)) }
    moreover
    { assume "\<not> P x"
    ...
    { assume "sbDom\<cdot>y \<noteq> sbDom\<cdot>x"
      ...
      { assume "\<not> P x \<and> sbDom\<cdot>y \<noteq> sbDom\<cdot>x \<and> \<not> P x"
        then have "(P x)\<leadsto>\<Squnion>n. iter_sbfix2 F n cs x \<sqsubseteq> (P y)\<leadsto>\<Squnion>n. iter_sbfix2 F n cs y"
...
lemma spfComp_test8: assumes "sercomp_well f1 f2" 
  ...
  proof -
    ...
    then show ?thesis
      by (simp add: Diff_Un Diff_triv I_def Un_Diff assms(1))
  qed
...
lemma spfComp_I_domf1_eq: assumes "sercomp_well f1 f2" 
  ...
  using assms(1) assms(2) I_def spfComp_test8 by blast
...
lemma spfComp_serial : assumes "sercomp_well f1 f2" 
  ...
  apply(rule sb_eq)
  apply (smt C_def assms(1) assms(2)inf_sup_ord(4) sbunionDom sbunion_restrict 
...
lemma spfComp_cInOc1:  assumes "parcomp_well f1 f2"
                          and "c \<in> spfRan\<cdot>f1" 
  shows "c \<in> Oc f1 f2"
  using L_def Oc_def assms(2) by blast
...
lemma spfComp_parallel : assumes" parcomp_well f1 f2"
  ...
  apply(rule sb_eq)
    apply (metis (mono_tags, lifting) C_def SPF_Comp.spfCompH2_itDom Un_upper2 assms(1) assms(2) 
...
lemma spfCompParallelGetch1: assumes "parcomp_well f1 f2"
  ...
  apply(simp_all add: assms)
  using assms(1) assms(3) L_def by blast+
...
proof -
  ...
  then show ?thesis
    by (simp add: I_def Un_Diff Un_Diff_Int assms)
qed
