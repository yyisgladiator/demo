...
(*
definition RecvMed1:: "bool stream \<Rightarrow> M TSPF" where
"RecvMed1 oracle \<equiv> Abs_TSPF (\<lambda>b. (tsbDom\<cdot>b = {ds}) \<leadsto>
    let medOut =  (outswitch22 (b . ds) oracle) in
  Abs_TSB [ ds \<mapsto> b . ds,
            dr \<mapsto> medOut, 
...
definition ONECOMP :: "M TSPS" where
"ONECOMP \<equiv> (SEND \<Otimes> MED1) \<Otimes> (RECV \<Otimes> MED2)"


(* Goal1 : *)
lemma assumes "f \<in> (Rep_TSPS ONECOMP)"
  shows "tsAbs\<cdot>((the ((Rep_TSPF f) tIn)) . abpOut ) = tsAbs\<cdot>tInHelper"
...
lemma 
  fixes f:: "M TSPF"
  assumes "f \<in> (Rep_TSPS ONECOMP)" and "tsbiDom\<cdot>input = {abpIn}" 
  shows "tsAbs\<cdot>((f \<rightharpoonup> input) . abpOut ) = tsAbs\<cdot>(input . abpIn)"
sorry(*
