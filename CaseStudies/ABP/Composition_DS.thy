...
lemma srcdups_smap_com_h: 
  "a \<noteq> aa \<Longrightarrow> srcdups\<cdot>(smap f\<cdot>(srcdups\<cdot>(\<up>a \<bullet> \<up>aa \<bullet> s))) 
     = smap f\<cdot>(srcdups\<cdot>(\<up>a \<bullet> \<up>aa \<bullet> s)) \<Longrightarrow> f aa \<noteq> f a"
  sorry
