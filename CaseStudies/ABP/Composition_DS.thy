...
(* property 2 *) (* provable with types? *)
lemma prop2: "#(tsAbs\<cdot>(tsRemDups\<cdot>(tsProjSnd\<cdot>ts))) \<le> #(tsAbs\<cdot>(tsProjFst\<cdot>(tsRemDups\<cdot>ts)))"
  sorry
...
lemma tsaltbitpro_inp2out:
  ...
  proof -
    ...
    hence "#(tsAbs\<cdot>(tsRemDups\<cdot>as)) \<le> #(tsAbs\<cdot>(tsProjFst\<cdot>(tsRemDups\<cdot>ds)))"
      using prop2 trans_lnle by blast
    ...
    have prop7: "#(tsAbs\<cdot>(tsProjFst\<cdot>(tsRemDups\<cdot>ds))) = #(tsAbs\<cdot>(tsRemDups\<cdot>(tsProjSnd\<cdot>ds)))"
      by (metis eq h1 order_class.order.antisym prop2 prop6)
    (* property 8 *)
