...
text{* Untimed version of the medium. *}
...
text {* Assumption for medium lemmata: #({True} \<ominus> ora)=\<infinity> *} (* Frage *)
...
text{* If the first element in the oracle is True then the current message is transmitted. *}
...
text{* If the first element in the oracle is False then the current message is not transmitted. *}
...
text{* Ticks in the message stream are ignored. *}
...
text{* Only if nothing is sent nothing is transmitted. *}
...
text{* If the message stream is empty then the untimed medium will not transmit anything. *}
lemma smed_bot1 [simp]: "sMed\<cdot>\<bottom>\<cdot>ora = \<bottom>"
  by (simp add: sMed_def)

text{* If the oracle is empty then the untimed medium will not transmit anything. *}
...
text{* Untimed medium without oracle will transmit all messages.*}
...
text{* The duplicates that are removed from the zipped stream are at least as much as the 
       duplicates that are removed from the second stream of the zipped stream. *}
  ...
  next
    ...
    then show ?case 
      ...
      next
        case (scons b bs)
        then show ?thesis
          proof (case_tac "a=b", simp_all)
            show "a = b \<Longrightarrow> s = \<up>b \<bullet> bs \<Longrightarrow> 
              #(srcdups\<cdot>(sprojsnd\<cdot>(\<up>b \<bullet> \<up>b \<bullet> bs))) \<le> #(sprojsnd\<cdot>(srcdups\<cdot>(\<up>b \<bullet> bs)))"
              proof (case_tac a, case_tac b, simp_all)
                show "\<And>aa ba aaa baa.  aa = aaa \<and> ba = baa \<Longrightarrow> s = \<up>(aaa, baa) \<bullet> bs \<Longrightarrow> 
                  a = (aaa, baa) \<Longrightarrow> b = (aaa, baa) \<Longrightarrow>
                  #(srcdups\<cdot>(\<up>baa \<bullet> sprojsnd\<cdot>bs)) \<le> #(sprojsnd\<cdot>(srcdups\<cdot>(\<up>(aaa, baa) \<bullet> bs)))"
                  using "3.IH" by auto 
              qed
            show  "a \<noteq> b \<Longrightarrow> s = \<up>b \<bullet> bs \<Longrightarrow> 
              #(srcdups\<cdot>(sprojsnd\<cdot>(\<up>a \<bullet> \<up>b \<bullet> bs))) \<le> #(sprojsnd\<cdot>(\<up>a \<bullet> srcdups\<cdot>(\<up>b \<bullet> bs)))"
              proof (case_tac a, case_tac b)
                show "\<And>aa ba aaa baa. 
                  a \<noteq> b \<Longrightarrow> s = \<up>b \<bullet> bs \<Longrightarrow> a = (aa, ba) \<Longrightarrow> b = (aaa, baa) \<Longrightarrow>
                  #(srcdups\<cdot>(sprojsnd\<cdot>(\<up>a \<bullet> \<up>b \<bullet> bs))) \<le> #(sprojsnd\<cdot>(\<up>a \<bullet> srcdups\<cdot>(\<up>b \<bullet> bs)))"
                  proof (case_tac "aa=aaa", simp_all)
                    show "\<And>aa ba aaa baa.  ba \<noteq> baa \<Longrightarrow> s = \<up>(aaa, baa) \<bullet> bs \<Longrightarrow> 
                      a = (aaa, ba) \<Longrightarrow> b = (aaa, baa) \<Longrightarrow> aa = aaa \<Longrightarrow> 
                      #(srcdups\<cdot>(\<up>baa \<bullet> sprojsnd\<cdot>bs)) \<le> #(sprojsnd\<cdot>(srcdups\<cdot>(\<up>(aaa, baa) \<bullet> bs)))"
                      using "3.IH" by auto
                    show "\<And>aa ba aaa baa. s = \<up>(aaa, baa) \<bullet> bs \<Longrightarrow> a = (aa, ba) \<Longrightarrow>
                      b = (aaa, baa) \<Longrightarrow> aa \<noteq> aaa \<Longrightarrow>
                      #(srcdups\<cdot>(\<up>ba \<bullet> \<up>baa \<bullet> sprojsnd\<cdot>bs)) \<le> lnsuc\<cdot>(#(sprojsnd\<cdot>(srcdups\<cdot>(\<up>(aaa, baa) \<bullet> bs))))"
                      proof (case_tac "ba=baa", simp_all)
                        show "\<And>aa ba aaa baa. s = \<up>(aaa, baa) \<bullet> bs \<Longrightarrow> a = (aa, baa) \<Longrightarrow>
                          b = (aaa, baa) \<Longrightarrow> aa \<noteq> aaa \<Longrightarrow> ba = baa \<Longrightarrow>
                          #(srcdups\<cdot>(\<up>baa \<bullet> sprojsnd\<cdot>bs)) \<le> lnsuc\<cdot>(#(sprojsnd\<cdot>(srcdups\<cdot>(\<up>(aaa, baa) \<bullet> bs))))"
                          by (metis (no_types, lifting) "3.IH" less_lnsuc sprojsnd_scons trans_lnle)
                        show "\<And>aa ba aaa baa. s = \<up>(aaa, baa) \<bullet> bs \<Longrightarrow> a = (aa, ba) \<Longrightarrow>
                          b = (aaa, baa) \<Longrightarrow> aa \<noteq> aaa \<Longrightarrow> ba \<noteq> baa \<Longrightarrow> 
                          #(srcdups\<cdot>(\<up>baa \<bullet> sprojsnd\<cdot>bs)) \<le> #(sprojsnd\<cdot>(srcdups\<cdot>(\<up>(aaa, baa) \<bullet> bs)))"
                          using "3.IH" by auto
                      qed
                  qed
...
lemma slen_dropwhile_takewhile: 
  ...
  using assms
  proof (erule_tac contrapos_pp, simp)
    assume inf: "#(stakewhile (\<lambda>p. p = a)\<cdot>s) = \<infinity>" 
    show "sdropwhile (\<lambda>x. x = a)\<cdot>s \<noteq> \<epsilon> \<Longrightarrow> False"
      using stakewhile_sinftimesup [of a s] inf dropwhile_inf_bot by blast 
  qed
...
(* Frage? *)
lemma szip_slen_conc: 
  "#ora \<le> #sa \<Longrightarrow> #sa = Fin k \<Longrightarrow> szip\<cdot>(sa \<bullet> sb)\<cdot>ora = szip\<cdot>sa\<cdot>ora"
  proof (induction k arbitrary: ora sa sb, simp_all)
    show "\<And>k ora sa sb. 
      (\<And>ora sa sb. #ora \<le> Fin k \<Longrightarrow> #sa = Fin k \<Longrightarrow> szip\<cdot>(sa \<bullet> sb)\<cdot>ora = szip\<cdot>sa\<cdot>ora) \<Longrightarrow>
      #ora \<le> Fin (Suc k) \<Longrightarrow> #sa = Fin (Suc k) \<Longrightarrow> szip\<cdot>(sa \<bullet> sb)\<cdot>ora = szip\<cdot>sa\<cdot>ora" 
      proof (rule_tac x=sa in scases,simp_all)
        show "\<And>k ora sa sb a s.
          (\<And>ora sa sb. #ora \<le> Fin k \<Longrightarrow> #sa = Fin k \<Longrightarrow> szip\<cdot>(sa \<bullet> sb)\<cdot>ora = szip\<cdot>sa\<cdot>ora) \<Longrightarrow>
          #ora \<le> Fin (Suc k) \<Longrightarrow>
          #s = Fin k \<Longrightarrow> sa = \<up>a \<bullet> s \<Longrightarrow> szip\<cdot>(\<up>a \<bullet> s \<bullet> sb)\<cdot>ora = szip\<cdot>(\<up>a \<bullet> s)\<cdot>ora"
          proof (rule_tac x=ora in scases, simp_all)
            show "\<And>k ora sa sb a s aa sc.
              (\<And>ora sa sb. #ora \<le> Fin k \<Longrightarrow> #sa = Fin k \<Longrightarrow> szip\<cdot>(sa \<bullet> sb)\<cdot>ora = szip\<cdot>sa\<cdot>ora) \<Longrightarrow>
              #sc \<le> Fin k \<Longrightarrow> #s = Fin k \<Longrightarrow> sa = \<up>a \<bullet> s \<Longrightarrow> ora = \<up>aa \<bullet> sc \<Longrightarrow> 
              \<up>(a, aa) \<bullet> szip\<cdot>(s \<bullet> sb)\<cdot>sc = \<up>(a, aa) \<bullet> szip\<cdot>s\<cdot>sc"
              (* Frage *)
              sorry
          qed
      qed
...
(* Frage *)
lemma szip_slen_conc_1: "#ora \<le> #sa \<Longrightarrow> #sa = Fin k \<Longrightarrow> szip\<cdot>(sa \<bullet> sb)\<cdot>ora =  szip\<cdot>sa\<cdot>ora"
...
lemma sdropwhile_conc: 
  ...
  proof (induction sa arbitrary: a sb rule: finind,simp_all)
    show "#sa = Fin k"
...
lemma prop0_h:"#(srcdups\<cdot>(s\<bullet>s2)) \<le> #(srcdups\<cdot>(s\<bullet>\<up>b\<bullet>s2))"
  ...
  next
    ...
    have case_epsneq:"a\<noteq>b \<Longrightarrow> #(srcdups\<cdot>(\<up>a \<bullet> s2)) \<le> #(srcdups\<cdot>(\<up>a \<bullet> \<up>b \<bullet> s2))"
      proof (subst srcdups_neq,simp)
        show "a \<noteq> b \<Longrightarrow> #(srcdups\<cdot>(\<up>a \<bullet> s2)) \<le> #(\<up>a \<bullet> srcdups\<cdot>(\<up>b \<bullet> s2))"
          proof (cases "s2=\<epsilon>", simp)
            show "a \<noteq> b \<Longrightarrow> s2 \<noteq> \<epsilon> \<Longrightarrow> #(srcdups\<cdot>(\<up>a \<bullet> s2)) \<le> #(\<up>a \<bullet> srcdups\<cdot>(\<up>b \<bullet> s2))"
              proof (cases "b=shd s2")
                show "a \<noteq> b \<Longrightarrow> s2 \<noteq> \<epsilon> \<Longrightarrow> b = shd s2 \<Longrightarrow> 
                  #(srcdups\<cdot>(\<up>a \<bullet> s2)) \<le> #(\<up>a \<bullet> srcdups\<cdot>(\<up>b \<bullet> s2))"
                  by (metis eq_refl srcdups_eq srcdups_neq surj_scons)
                show "a \<noteq> b \<Longrightarrow> s2 \<noteq> \<epsilon> \<Longrightarrow> b \<noteq> shd s2 \<Longrightarrow> 
                  #(srcdups\<cdot>(\<up>a \<bullet> s2)) \<le> #(\<up>a \<bullet> srcdups\<cdot>(\<up>b \<bullet> s2))"
                    ...
                    then show ?thesis
                      ...
                      next
                        ...
                        then have g7: "shd (tsAbs\<cdot>(tsMed\<cdot>ts\<cdot>(\<up>True \<bullet> srt\<cdot>(srt\<cdot>p)))) = 
                          ...
                          (* by (metis tsmed_shd sinftimes_unfold deconstruct_infstream_h lscons_conv 
                              mlscons.prems slen_sinftimes stream.con_rews(2) stream.sel_rews(5) 
                              sup'_def up_defined)*)
                          sorry
