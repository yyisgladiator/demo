 ...
 *
 * Generated on Sep 22, 2018 7:02:42 PM by isartransformer 1.0.0
 *)
theory BooleanMediumAutomaton
...
datatype mediumMessage = MediumBool "bool"

instance mediumMessage :: countable
  apply(intro_classes)
  by(countable_datatype)

instantiation mediumMessage :: message
begin
  fun ctype_mediumMessage :: "channel \<Rightarrow> mediumMessage set" where
  "ctype_mediumMessage c = (
    if c = \<C> ''ar'' then range MediumBool else
    if c = \<C> ''as'' then range MediumBool else
    undefined)"
  instance
    by(intro_classes)
end
...
lift_definition mediumElem_raw_ar :: "bool \<Rightarrow> mediumMessage tsyn sbElem" is
"\<lambda>x. [\<C> ''ar'' \<mapsto> Msg (MediumBool x)]"
  unfolding sbElemWell_def
  unfolding usclOkay_stream_def
  unfolding ctype_tsyn_def
  by simp

lift_definition mediumElem_raw_as :: "bool \<Rightarrow> mediumMessage tsyn sbElem" is
"\<lambda>x. [\<C> ''as'' \<mapsto> Msg (MediumBool x)]"
  unfolding sbElemWell_def
  unfolding usclOkay_stream_def
  unfolding ctype_tsyn_def
  by simp
...
fun medium_list_ar :: "(bool tsyn) list \<Rightarrow> mediumMessage tsyn SB" where
"medium_list_ar (x#xs) = ubConcEq (medium_ar x)\<cdot>(medium_list_ar xs)" |
"medium_list_ar []     = ubLeast {\<C> ''ar''}"

fun medium_list_as :: "(bool tsyn) list \<Rightarrow> mediumMessage tsyn SB" where
"medium_list_as (x#xs) = ubConcEq (medium_as x)\<cdot>(medium_list_as xs)" |
"medium_list_as []     = ubLeast {\<C> ''as''}"

(* Create one SB for all input channels *)
definition mediumIn_list_ar :: "bool tsyn list \<Rightarrow> mediumMessage tsyn SB" where
"mediumIn_list_ar port_ar = (medium_list_ar port_ar)"

(* Create one SB for all output channels *)
definition mediumOut_list_as :: "bool tsyn list \<Rightarrow> mediumMessage tsyn SB" where
...
lift_definition medium_stream_ar_h :: "bool tsyn stream \<Rightarrow> mediumMessage tsyn SB" is
"\<lambda> s. [(\<C> ''ar'') \<mapsto> (tsynMap (MediumBool)\<cdot>s)]"
  unfolding ubWell_def usclOkay_stream_def ctype_tsyn_def
  apply auto
  sorry

lift_definition medium_stream_ar :: "(bool) tsyn stream \<rightarrow> mediumMessage tsyn SB" is
"medium_stream_ar_h"
  sorry

lift_definition medium_stream_as_h :: "bool tsyn stream \<Rightarrow> mediumMessage tsyn SB" is
"\<lambda> s. [(\<C> ''as'') \<mapsto> (tsynMap (MediumBool)\<cdot>s)]"
  unfolding ubWell_def usclOkay_stream_def ctype_tsyn_def
  apply auto
  sorry

lift_definition medium_stream_as :: "(bool) tsyn stream \<rightarrow> mediumMessage tsyn SB" is
"medium_stream_as_h"
  sorry

(* Create one SB for all input channels *)
definition mediumIn_stream_ar :: "bool tsyn stream \<rightarrow> mediumMessage tsyn SB" where
"mediumIn_stream_ar = (\<Lambda>  port_ar. (medium_stream_ar\<cdot>port_ar))"

(* Create one SB for all output channels *)
definition mediumOut_stream_as :: "bool tsyn stream \<rightarrow> mediumMessage tsyn SB" where
...
fun mediumElem_get_ar :: "mediumMessage tsyn sbElem \<Rightarrow> (bool) tsyn" where
"mediumElem_get_ar sbe = undefined"

lift_definition medium_get_stream_ar :: "mediumMessage tsyn SB \<rightarrow> bool tsyn stream" is
"\<lambda>sb. tsynMap (inv MediumBool)\<cdot>(sb . (\<C> ''ar''))"
  by(simp add: cfun_def)

fun mediumElem_get_as :: "mediumMessage tsyn sbElem \<Rightarrow> (bool) tsyn" where
"mediumElem_get_as sbe = undefined"

lift_definition medium_get_stream_as :: "mediumMessage tsyn SB \<rightarrow> bool tsyn stream" is
"\<lambda>sb. tsynMap (inv MediumBool)\<cdot>(sb . (\<C> ''as''))"
  by(simp add: cfun_def)
...
fun mediumTransitionH :: "(MediumState \<times> (bool tsyn)) \<Rightarrow> (MediumState \<times> mediumMessage tsyn SB) set rev" where
...
(* Transition function *)
definition mediumTransition :: "(MediumState \<times> mediumMessage tsyn sbElem) \<Rightarrow> (MediumState \<times> mediumMessage tsyn SB) set rev" where
...
definition mediumInitials :: "(MediumState \<times> mediumMessage tsyn SB) set rev" where
"mediumInitials = Rev (setflat\<cdot>{{(MediumState Single var_c, (mediumOut_as null)) | var_c . (True)}})"

(* The final automaton *)
lift_definition mediumAutomaton :: "(MediumState, mediumMessage tsyn) ndAutomaton" is
...
definition mediumStep :: "MediumState \<Rightarrow> mediumMessage tsyn SPS" where
"mediumStep = nda_h mediumAutomaton"

(* The final SPF *)
definition mediumSPS :: "mediumMessage tsyn SPS" where
"mediumSPS = nda_H mediumAutomaton"
...
lemma mediumSps2Step: "mediumSPS = uspecFlatten mediumDom mediumRan
    (Rev {spsConcOut (mediumOut_as null)\<cdot>(mediumStep (MediumState Single var_c)) | var_c . (True)})"
sorry
...
lemma mediumStep_0_0:
  assumes "var_c>0"
    shows "spsConcIn  (mediumIn_ar (Msg port_ar)) (mediumStep (MediumState Single var_c))
         = spsConcOut (mediumOut_as null)\<cdot>(mediumStep (MediumState Single (var_c-1)))"
  oops
  ...
  assumes "var_c=0"
    shows "spsConcIn  (mediumIn_ar (Msg port_ar)) (mediumStep (MediumState Single var_c))
         = uspecFlatten mediumDom mediumRan
          (Rev {spsConcOut (mediumOut_as (Msg (port_ar)))\<cdot>(mediumStep (MediumState Single var_c)) | var_c . (True)})"
  oops
