...
fun createTsynAsBundle :: "bool tsyn \<Rightarrow> senderMessage tsyn sbElem" where
"createTsynAsBundle (Msg x) = (createAsBundle x)" |
"createTsynAsBundle null = (sbeNull {\<C> ''as''})"

fun createTsynIBundle :: "nat tsyn \<Rightarrow> senderMessage tsyn sbElem" where
"createTsynIBundle (Msg x) = (createIBundle x)" |
"createTsynIBundle null = (sbeNull {\<C> ''i''})"

fun createTsynDsBundle :: "(nat\<times>bool) tsyn \<Rightarrow> senderMessage tsyn sbElem" where
"createTsynDsBundle (Msg x) = (createDsBundle x)" |
"createTsynDsBundle null = (sbeNull {\<C> ''ds''})"
...
"createLongAsBundle (x#xs) = ubConcEq (sbe2SB (createTsynAsBundle x))\<cdot>(createLongAsBundle xs)" |
"createLongAsBundle []     = ubLeast {\<C> ''as''}"

fun createLongIBundle :: "(nat tsyn) list \<Rightarrow> senderMessage tsyn SB" where
"createLongIBundle (x#xs) = ubConcEq (sbe2SB (createTsynIBundle x))\<cdot>(createLongIBundle xs)" |
"createLongIBundle []     = ubLeast {\<C> ''i''}"

fun createLongDsBundle :: "((nat\<times>bool) tsyn) list \<Rightarrow> senderMessage tsyn SB" where
"createLongDsBundle (x#xs) = ubConcEq (sbe2SB (createTsynDsBundle x))\<cdot>(createLongDsBundle xs)" |
...
lemma createTsynAsBundle_dom [simp]: "sbeDom (createTsynAsBundle x) = {\<C> ''as''}"
  by (cases x, simp_all)
...
lemma createTsynIBundle_dom [simp]: "sbeDom (createTsynIBundle x) = {\<C> ''i''}"
  by (cases x, simp_all)
...
lemma createTsynDsBundle_dom [simp]: "sbeDom (createTsynDsBundle x) = {\<C> ''ds''}"
  by (cases x, simp_all)
...
lemma tbd_0_0:
  assumes "(size var_buffer)>1 \<and> port_as=False"
    shows "spfConcIn  (sbe2DB (createAsBundle port_as) \<uplus> sbe2SB (createIBundle port_i))\<cdot>(da_h SenderAutomaton (SenderState Sf var_buffer var_c))
...
lemma tbd_0_2:
  assumes "(size var_buffer)=0"
    shows "spfConcIn  (sbe2SB (createAsBundle port_as) \<uplus> sbe2SB (createIBundle port_i))\<cdot>(da_h SenderAutomaton (SenderState Sf var_buffer var_c))
...
lemma tbd_1_0:
  assumes "(size var_buffer)>1 \<and> port_as=False"
    shows "spfConcIn  (sbe2DB (createAsBundle port_as) \<uplus> tsynbNull (\<C> ''i''))\<cdot>(da_h SenderAutomaton (SenderState Sf var_buffer var_c))
...
lemma tbd_1_2:
  assumes "(size var_buffer)=0"
    shows "spfConcIn  (sbe2SB (createAsBundle port_as) \<uplus> tsynbNull (\<C> ''i''))\<cdot>(da_h SenderAutomaton (SenderState Sf var_buffer var_c))
...
lemma tbd_2_0:
  assumes "(size var_buffer)=0"
    shows "spfConcIn  (tsynbNull (\<C> ''as'') \<uplus> sbe2SB (createIBundle port_i))\<cdot>(da_h SenderAutomaton (SenderState Sf var_buffer var_c))
         = spfConcOut (sbe2SB (createDsBundle (Pair port_i False)))\<cdot>(da_h SenderAutomaton (SenderState Sf (prepend var_buffer port_i) 3))"
  oops
...
lemma tbd_4_0:
  assumes "(size var_buffer)=0"
    shows "spfConcIn  (sbe2SB (createAsBundle port_as) \<uplus> sbe2SB (createIBundle port_i))\<cdot>(da_h SenderAutomaton (SenderState St var_buffer var_c))
...
lemma tbd_4_1:
  assumes "(size var_buffer)>1 \<and> port_as=True"
    shows "spfConcIn  (sbe2DB (createAsBundle port_as) \<uplus> sbe2SB (createIBundle port_i))\<cdot>(da_h SenderAutomaton (SenderState St var_buffer var_c))
...
lemma tbd_5_0:
  assumes "(size var_buffer)=0"
    shows "spfConcIn  (sbe2SB (createAsBundle port_as) \<uplus> tsynbNull (\<C> ''i''))\<cdot>(da_h SenderAutomaton (SenderState St var_buffer var_c))
...
lemma tbd_5_1:
  assumes "(size var_buffer)>1 \<and> port_as=True"
    shows "spfConcIn  (sbe2DB (createAsBundle port_as) \<uplus> tsynbNull (\<C> ''i''))\<cdot>(da_h SenderAutomaton (SenderState St var_buffer var_c))
...
lemma tbd_6_0:
  assumes "(size var_buffer)=0"
    shows "spfConcIn  (tsynbNull (\<C> ''as'') \<uplus> sbe2SB (createIBundle port_i))\<cdot>(da_h SenderAutomaton (SenderState St var_buffer var_c))
         = spfConcOut (sbe2SB (createDsBundle (Pair port_i True)))\<cdot>(da_h SenderAutomaton (SenderState St (prepend var_buffer port_i) 3))"
  oops
