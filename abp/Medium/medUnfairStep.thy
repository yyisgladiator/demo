...
lemma "spsConcIn (medIn (Msg m)) (medUnfair (Suc n)) = spsConcOut (medOut -)(medUnfair n)"
  oops

(* If a "null" comes in send it out and stay in the same state *) 
lemma "spsConcIn (medIn -) (medUnfair state) = spsConcOut (medOut -)(medUnfair state)"
  oops

(* Counter hit zero, so pass the message and reset the countdown to a random value *)
lemma "spsConcIn (medIn (Msg m)) (medUnfair 0) 
  = uspecUnion\<cdot>
      (spsConcOut (medOut (Msg m))(uspecFlatten medInDom medOutDom (Rev {medUnfair n |  n. True})))\<cdot>
      (spsConcOut (medOut (Msg m))(medUnfair 0))"
  oops
