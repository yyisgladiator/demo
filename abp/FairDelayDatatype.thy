...
datatype ('e::countable) fairDelayMessage = FairDelayE "'e"
...
begin
  ...
  "ctype_fairDelayMessage c = (
    if c = \<C> ''i'' then range FairDelayE else
    if c = \<C> ''o'' then range FairDelayE else
    undefined)"
  instance
    by(intro_classes)
end
...
"\<lambda>x. [\<C> ''i'' \<mapsto> Msg (FairDelayE x)]"
...
"\<lambda>x. [\<C> ''o'' \<mapsto> Msg (FairDelayE x)]"
  unfolding sbElemWell_def usclOkay_stream_def ctype_tsyn_def
  by simp
...
"fairDelayElem_i null = sbeNull {\<C> ''i''}"
...
"fairDelayElem_o null = sbeNull {\<C> ''o''}"
...
lift_definition fairDelay_stream_i_h :: "'e tsyn stream \<Rightarrow> ('e::countable) fairDelayMessage tsyn SB" is
"\<lambda> s. [(\<C> ''i'') \<mapsto> (tsynMap (FairDelayE)\<cdot>s)]"
...
"fairDelay_stream_i_h"
  apply(auto simp add: cfun_def fairDelay_stream_i_h_def map_fun_def comp_def)
  apply(rule cont_Abs_UB)
  apply(simp add: option_one_cont)
  by (metis fairDelay_stream_i_h.rep_eq ubrep_well)

lift_definition fairDelay_stream_o_h :: "'e tsyn stream \<Rightarrow> ('e::countable) fairDelayMessage tsyn SB" is
"\<lambda> s. [(\<C> ''o'') \<mapsto> (tsynMap (FairDelayE)\<cdot>s)]"
  unfolding ubWell_def usclOkay_stream_def ctype_tsyn_def
  by auto
...
"fairDelay_stream_o_h"
  apply(auto simp add: cfun_def fairDelay_stream_o_h_def map_fun_def comp_def)
  apply(rule cont_Abs_UB)
  apply(simp add: option_one_cont)
  by (metis fairDelay_stream_o_h.rep_eq ubrep_well)
...
"fairDelayElem_get_i sbe = tsynApplyElem (inv FairDelayE) ((Rep_sbElem sbe) \<rightharpoonup> (\<C> ''i''))"

definition fairDelayElem_get_o :: "('e::countable) fairDelayMessage tsyn sbElem \<Rightarrow> ('e) tsyn" where
"fairDelayElem_get_o sbe = tsynApplyElem (inv FairDelayE) ((Rep_sbElem sbe) \<rightharpoonup> (\<C> ''o''))"
...
lemma fairdelay_stream_i_dom[simp]: "ubDom\<cdot>(fairDelay_stream_i\<cdot>x) = {\<C> ''i''}"
  by(simp add: fairDelay_stream_i.rep_eq ubdom_insert fairDelay_stream_i_h.rep_eq)
...
lemma fairdelay_stream_i_len[simp]: "ubLen (fairDelay_stream_i\<cdot>x) = #x"
  ...
  apply(simp add: ubGetCh_def fairDelay_stream_i_h.rep_eq)
...
lemma fairdelay_stream_i_id[simp]:
  ...
  apply(simp add: fairDelay_stream_i_h_def)
  apply(subst tsynmap_inv_id)
  using assms tsynbundle_ctype apply fastforce
  using assms ub_id_single by blast

lemma fairdelay_stream_o_dom[simp]: "ubDom\<cdot>(fairDelay_stream_o\<cdot>x) = {\<C> ''o''}"
  by(simp add: fairDelay_stream_o.rep_eq ubdom_insert fairDelay_stream_o_h.rep_eq)

lemma fairdelay_stream_o_len[simp]: "ubLen (fairDelay_stream_o\<cdot>x) = #x"
  apply(subst uslen_ubLen_ch3)
  apply simp
  apply(simp add: fairDelay_stream_o.rep_eq)
  apply(simp add: ubGetCh_def fairDelay_stream_o_h.rep_eq)
  by (simp add: tsynmap_slen usclLen_stream_def)
...
lemma fairdelay_stream_o_id[simp]:
  ...
  apply(simp add: fairDelay_stream_o_h_def)
  apply(subst tsynmap_inv_id)
  using assms tsynbundle_ctype apply fastforce
  using assms ub_id_single by blast
