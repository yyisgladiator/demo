...
datatype ('e::countable) fairDelayMessage = DoNotUse_3cfc59_FairDelayE "'e"
...
begin
  ...
  "ctype_fairDelayMessage c = (
    if c = \<C> ''DoNotUse_3cfc59_i'' then range DoNotUse_3cfc59_FairDelayE else
    if c = \<C> ''DoNotUse_3cfc59_o'' then range DoNotUse_3cfc59_FairDelayE else
    undefined)"
  instance
    by(intro_classes)
end
...
"fairDelayDom = {\<C> ''DoNotUse_3cfc59_i''}"

definition fairDelayRan :: "channel set" where
"fairDelayRan = {\<C> ''DoNotUse_3cfc59_o''}"
...
"\<lambda>x. [\<C> ''DoNotUse_3cfc59_i'' \<mapsto> Msg (DoNotUse_3cfc59_FairDelayE x)]"
...
"\<lambda>x. [\<C> ''DoNotUse_3cfc59_o'' \<mapsto> Msg (DoNotUse_3cfc59_FairDelayE x)]"
  unfolding sbElemWell_def usclOkay_stream_def ctype_tsyn_def
  by simp
...
"fairDelayElem_i null = sbeNull {\<C> ''DoNotUse_3cfc59_i''}"
...
"fairDelayElem_o null = sbeNull {\<C> ''DoNotUse_3cfc59_o''}"
...
"fairDelay_list_i []     = ubLeast {\<C> ''DoNotUse_3cfc59_i''}"
...
"fairDelay_list_o []     = ubLeast {\<C> ''DoNotUse_3cfc59_o''}"
...
lift_definition DoNotUse_3cfc59_fairDelay_stream_i_h :: "'e tsyn stream \<Rightarrow> ('e::countable) fairDelayMessage tsyn SB" is
"\<lambda> s. [(\<C> ''DoNotUse_3cfc59_i'') \<mapsto> (tsynMap (DoNotUse_3cfc59_FairDelayE)\<cdot>s)]"
...
"DoNotUse_3cfc59_fairDelay_stream_i_h"
  apply(auto simp add: cfun_def DoNotUse_3cfc59_fairDelay_stream_i_h_def map_fun_def comp_def)
  apply(rule cont_Abs_UB)
  apply(simp add: option_one_cont)
  by (metis DoNotUse_3cfc59_fairDelay_stream_i_h.rep_eq ubrep_well)

lift_definition DoNotUse_3cfc59_fairDelay_stream_o_h :: "'e tsyn stream \<Rightarrow> ('e::countable) fairDelayMessage tsyn SB" is
"\<lambda> s. [(\<C> ''DoNotUse_3cfc59_o'') \<mapsto> (tsynMap (DoNotUse_3cfc59_FairDelayE)\<cdot>s)]"
  unfolding ubWell_def usclOkay_stream_def ctype_tsyn_def
  by auto
...
"DoNotUse_3cfc59_fairDelay_stream_o_h"
  apply(auto simp add: cfun_def DoNotUse_3cfc59_fairDelay_stream_o_h_def map_fun_def comp_def)
  apply(rule cont_Abs_UB)
  apply(simp add: option_one_cont)
  by (metis DoNotUse_3cfc59_fairDelay_stream_o_h.rep_eq ubrep_well)
...
"fairDelayElem_get_i sbe = tsynApplyElem (inv DoNotUse_3cfc59_FairDelayE) ((Rep_sbElem sbe) \<rightharpoonup> (\<C> ''DoNotUse_3cfc59_i''))"

definition fairDelayElem_get_o :: "('e::countable) fairDelayMessage tsyn sbElem \<Rightarrow> ('e) tsyn" where
"fairDelayElem_get_o sbe = tsynApplyElem (inv DoNotUse_3cfc59_FairDelayE) ((Rep_sbElem sbe) \<rightharpoonup> (\<C> ''DoNotUse_3cfc59_o''))"
...
"\<lambda>sb. tsynMap (inv DoNotUse_3cfc59_FairDelayE)\<cdot>(sb . (\<C> ''DoNotUse_3cfc59_i''))"
  by(simp add: cfun_def)

lift_definition fairDelay_get_stream_o :: "('e::countable) fairDelayMessage tsyn SB \<rightarrow> 'e tsyn stream" is
"\<lambda>sb. tsynMap (inv DoNotUse_3cfc59_FairDelayE)\<cdot>(sb . (\<C> ''DoNotUse_3cfc59_o''))"
  by(simp add: cfun_def)
...
lemma fairdelayelem_i_dom[simp]: "sbeDom (fairDelayElem_i x) = {\<C> ''DoNotUse_3cfc59_i''}"
  apply(cases x)
  apply(simp add: fairDelayElem_i.simps sbeDom_def fairDelayElem_raw_i.rep_eq)
  by(simp add: fairDelayElem_i.simps)

lemma fairdelayelem_o_dom[simp]: "sbeDom (fairDelayElem_o x) = {\<C> ''DoNotUse_3cfc59_o''}"
  apply(cases x)
  apply(simp add: fairDelayElem_o.simps sbeDom_def fairDelayElem_raw_o.rep_eq)
  by(simp add: fairDelayElem_o.simps)

lemma fairdelay_i_dom[simp]: "ubDom\<cdot>(fairDelay_i x) = {\<C> ''DoNotUse_3cfc59_i''}"
  by(simp add: fairDelay_i_def)

lemma fairdelay_i_len[simp]: "ubLen (fairDelay_i x) = 1"
  by(simp add: fairDelay_i_def)

lemma fairdelay_o_dom[simp]: "ubDom\<cdot>(fairDelay_o x) = {\<C> ''DoNotUse_3cfc59_o''}"
...
lemma fairdelay_stream_i_dom[simp]: "ubDom\<cdot>(fairDelay_stream_i\<cdot>x) = {\<C> ''DoNotUse_3cfc59_i''}"
  by(simp add: fairDelay_stream_i.rep_eq ubdom_insert DoNotUse_3cfc59_fairDelay_stream_i_h.rep_eq)
...
lemma fairdelay_stream_i_len[simp]: "ubLen (fairDelay_stream_i\<cdot>x) = #x"
  ...
  apply(simp add: ubGetCh_def DoNotUse_3cfc59_fairDelay_stream_i_h.rep_eq)
  by (simp add: tsynmap_slen usclLen_stream_def)

lemma fairdelay_stream_i_id[simp]:
  assumes "ubDom\<cdot>ub = {\<C> ''DoNotUse_3cfc59_i''} "
  ...
  apply(simp add: DoNotUse_3cfc59_fairDelay_stream_i_h_def)
  apply(subst tsynmap_inv_id)
  using assms tsynbundle_ctype apply fastforce
  using assms ub_id_single by blast

lemma fairdelay_stream_o_dom[simp]: "ubDom\<cdot>(fairDelay_stream_o\<cdot>x) = {\<C> ''DoNotUse_3cfc59_o''}"
  by(simp add: fairDelay_stream_o.rep_eq ubdom_insert DoNotUse_3cfc59_fairDelay_stream_o_h.rep_eq)

lemma fairdelay_stream_o_len[simp]: "ubLen (fairDelay_stream_o\<cdot>x) = #x"
  apply(subst uslen_ubLen_ch3)
  apply simp
  apply(simp add: fairDelay_stream_o.rep_eq)
  apply(simp add: ubGetCh_def DoNotUse_3cfc59_fairDelay_stream_o_h.rep_eq)
  by (simp add: tsynmap_slen usclLen_stream_def)

lemma fairdelay_stream_o_id[simp]:
  assumes "ubDom\<cdot>ub = {\<C> ''DoNotUse_3cfc59_o''} "
  ...
  apply(simp add: DoNotUse_3cfc59_fairDelay_stream_o_h_def)
  apply(subst tsynmap_inv_id)
  using assms tsynbundle_ctype apply fastforce
  using assms ub_id_single by blast
...
lemma fairdelay_get_stream_i_eq:
  assumes "ubDom\<cdot>ub1 = {\<C> ''DoNotUse_3cfc59_i''}"
      and "ubDom\<cdot>ub2 = {\<C> ''DoNotUse_3cfc59_i''}"
...
lemma fairdelay_get_stream_i_conc[simp]:
  assumes "\<C> ''DoNotUse_3cfc59_i'' \<in> ubDom\<cdot>ub1"
      and "\<C> ''DoNotUse_3cfc59_i'' \<in> ubDom\<cdot>ub2"
...
lemma fairdelay_get_stream_o_eq:
  assumes "ubDom\<cdot>ub1 = {\<C> ''DoNotUse_3cfc59_o''}"
      and "ubDom\<cdot>ub2 = {\<C> ''DoNotUse_3cfc59_o''}"
...
lemma fairdelay_get_stream_o_conc[simp]:
  assumes "\<C> ''DoNotUse_3cfc59_o'' \<in> ubDom\<cdot>ub1"
      and "\<C> ''DoNotUse_3cfc59_o'' \<in> ubDom\<cdot>ub2"
