...
datatype ('e::countable) mediumMessage = DoNotUse_e925dc_MediumE "'e"
...
begin
  ...
  "ctype_mediumMessage c = (
    if c = \<C> ''DoNotUse_e925dc_i'' then range DoNotUse_e925dc_MediumE else
    if c = \<C> ''DoNotUse_e925dc_o'' then range DoNotUse_e925dc_MediumE else
    undefined)"
  instance
    by(intro_classes)
end
...
"mediumDom = {\<C> ''DoNotUse_e925dc_i''}"

definition mediumRan :: "channel set" where
"mediumRan = {\<C> ''DoNotUse_e925dc_o''}"
...
"\<lambda>x. [\<C> ''DoNotUse_e925dc_i'' \<mapsto> Msg (DoNotUse_e925dc_MediumE x)]"
...
"\<lambda>x. [\<C> ''DoNotUse_e925dc_o'' \<mapsto> Msg (DoNotUse_e925dc_MediumE x)]"
  unfolding sbElemWell_def usclOkay_stream_def ctype_tsyn_def
  by simp
...
"mediumElem_i null = sbeNull {\<C> ''DoNotUse_e925dc_i''}"
...
"mediumElem_o null = sbeNull {\<C> ''DoNotUse_e925dc_o''}"
...
"medium_list_i []     = ubLeast {\<C> ''DoNotUse_e925dc_i''}"
...
"medium_list_o []     = ubLeast {\<C> ''DoNotUse_e925dc_o''}"
...
lift_definition DoNotUse_e925dc_medium_stream_i_h :: "'e tsyn stream \<Rightarrow> ('e::countable) mediumMessage tsyn SB" is
"\<lambda> s. [(\<C> ''DoNotUse_e925dc_i'') \<mapsto> (tsynMap (DoNotUse_e925dc_MediumE)\<cdot>s)]"
...
"DoNotUse_e925dc_medium_stream_i_h"
  apply(auto simp add: cfun_def DoNotUse_e925dc_medium_stream_i_h_def map_fun_def comp_def)
  apply(rule cont_Abs_UB)
  apply(simp add: option_one_cont)
  by (metis DoNotUse_e925dc_medium_stream_i_h.rep_eq ubrep_well)

lift_definition DoNotUse_e925dc_medium_stream_o_h :: "'e tsyn stream \<Rightarrow> ('e::countable) mediumMessage tsyn SB" is
"\<lambda> s. [(\<C> ''DoNotUse_e925dc_o'') \<mapsto> (tsynMap (DoNotUse_e925dc_MediumE)\<cdot>s)]"
  unfolding ubWell_def usclOkay_stream_def ctype_tsyn_def
  by auto
...
"DoNotUse_e925dc_medium_stream_o_h"
  apply(auto simp add: cfun_def DoNotUse_e925dc_medium_stream_o_h_def map_fun_def comp_def)
  apply(rule cont_Abs_UB)
  apply(simp add: option_one_cont)
  by (metis DoNotUse_e925dc_medium_stream_o_h.rep_eq ubrep_well)
...
"mediumElem_get_i sbe = tsynApplyElem (inv DoNotUse_e925dc_MediumE) ((Rep_sbElem sbe) \<rightharpoonup> (\<C> ''DoNotUse_e925dc_i''))"

definition mediumElem_get_o :: "('e::countable) mediumMessage tsyn sbElem \<Rightarrow> ('e) tsyn" where
"mediumElem_get_o sbe = tsynApplyElem (inv DoNotUse_e925dc_MediumE) ((Rep_sbElem sbe) \<rightharpoonup> (\<C> ''DoNotUse_e925dc_o''))"
...
"\<lambda>sb. tsynMap (inv DoNotUse_e925dc_MediumE)\<cdot>(sb . (\<C> ''DoNotUse_e925dc_i''))"
  by(simp add: cfun_def)

lift_definition medium_get_stream_o :: "('e::countable) mediumMessage tsyn SB \<rightarrow> 'e tsyn stream" is
"\<lambda>sb. tsynMap (inv DoNotUse_e925dc_MediumE)\<cdot>(sb . (\<C> ''DoNotUse_e925dc_o''))"
  by(simp add: cfun_def)
...
lemma mediumelem_i_dom[simp]: "sbeDom (mediumElem_i x) = {\<C> ''DoNotUse_e925dc_i''}"
  apply(cases x)
  apply(simp add: mediumElem_i.simps sbeDom_def mediumElem_raw_i.rep_eq)
  by(simp add: mediumElem_i.simps)

lemma mediumelem_o_dom[simp]: "sbeDom (mediumElem_o x) = {\<C> ''DoNotUse_e925dc_o''}"
  apply(cases x)
  apply(simp add: mediumElem_o.simps sbeDom_def mediumElem_raw_o.rep_eq)
  by(simp add: mediumElem_o.simps)

lemma medium_i_dom[simp]: "ubDom\<cdot>(medium_i x) = {\<C> ''DoNotUse_e925dc_i''}"
  by(simp add: medium_i_def)

lemma medium_i_len[simp]: "ubLen (medium_i x) = 1"
  by(simp add: medium_i_def)

lemma medium_o_dom[simp]: "ubDom\<cdot>(medium_o x) = {\<C> ''DoNotUse_e925dc_o''}"
...
lemma medium_stream_i_dom[simp]: "ubDom\<cdot>(medium_stream_i\<cdot>x) = {\<C> ''DoNotUse_e925dc_i''}"
  by(simp add: medium_stream_i.rep_eq ubdom_insert DoNotUse_e925dc_medium_stream_i_h.rep_eq)
...
lemma medium_stream_i_len[simp]: "ubLen (medium_stream_i\<cdot>x) = #x"
  ...
  apply(simp add: ubGetCh_def DoNotUse_e925dc_medium_stream_i_h.rep_eq)
  by (simp add: tsynmap_slen usclLen_stream_def)

lemma medium_stream_i_id[simp]:
  assumes "ubDom\<cdot>ub = {\<C> ''DoNotUse_e925dc_i''} "
  ...
  apply(simp add: DoNotUse_e925dc_medium_stream_i_h_def)
  apply(subst tsynmap_inv_id)
  using assms tsynbundle_ctype apply fastforce
  using assms ub_id_single by blast

lemma medium_stream_o_dom[simp]: "ubDom\<cdot>(medium_stream_o\<cdot>x) = {\<C> ''DoNotUse_e925dc_o''}"
  by(simp add: medium_stream_o.rep_eq ubdom_insert DoNotUse_e925dc_medium_stream_o_h.rep_eq)

lemma medium_stream_o_len[simp]: "ubLen (medium_stream_o\<cdot>x) = #x"
  apply(subst uslen_ubLen_ch3)
  apply simp
  apply(simp add: medium_stream_o.rep_eq)
  apply(simp add: ubGetCh_def DoNotUse_e925dc_medium_stream_o_h.rep_eq)
  by (simp add: tsynmap_slen usclLen_stream_def)

lemma medium_stream_o_id[simp]:
  assumes "ubDom\<cdot>ub = {\<C> ''DoNotUse_e925dc_o''} "
  ...
  apply(simp add: DoNotUse_e925dc_medium_stream_o_h_def)
  apply(subst tsynmap_inv_id)
  using assms tsynbundle_ctype apply fastforce
  using assms ub_id_single by blast
...
lemma medium_get_stream_i_eq:
  assumes "ubDom\<cdot>ub1 = {\<C> ''DoNotUse_e925dc_i''}"
      and "ubDom\<cdot>ub2 = {\<C> ''DoNotUse_e925dc_i''}"
...
lemma medium_get_stream_i_conc[simp]:
  assumes "\<C> ''DoNotUse_e925dc_i'' \<in> ubDom\<cdot>ub1"
      and "\<C> ''DoNotUse_e925dc_i'' \<in> ubDom\<cdot>ub2"
...
lemma medium_get_stream_o_eq:
  assumes "ubDom\<cdot>ub1 = {\<C> ''DoNotUse_e925dc_o''}"
      and "ubDom\<cdot>ub2 = {\<C> ''DoNotUse_e925dc_o''}"
...
lemma medium_get_stream_o_conc[simp]:
  assumes "\<C> ''DoNotUse_e925dc_o'' \<in> ubDom\<cdot>ub1"
      and "\<C> ''DoNotUse_e925dc_o'' \<in> ubDom\<cdot>ub2"
