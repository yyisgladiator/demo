...
lemma [simp]: assumes "tspf_well F" shows "Rep_TSPF (Abs_TSPF F) = F"
using Abs_TSPF_inverse assms tspf_well_def by auto

(* Show that Map.empty is not an SPF *)
lemma map_not_tspf [simp]: "\<not>(tspf_well empty)"
...
lemma tspfcomp_well [simp]: "tspf_well (tspfCompHelper f1 f2)"
...
lemma [simp]: "tspf_well (tspfCompHelper2 f1 f2)"
sorry
...
*)
  ...
  cpodef 'm TSPFw = "{F :: 'm TSB_inf \<rightharpoonup> 'm TSB_inf. tspfw_well F}"
    apply (metis Rep_TSPF mem_Collect_eq tspf_StrongImpWeak tspf_well_def tspfw_well_def)
  by simp
