...
lemma uspec_sercomp_bla1: assumes "uspec_sercompwell S1 S2"
  ...
  proof auto
    ...
    proof -
      ...
      then show "\<exists>(f1a::'a) f2a::'a. f1 \<circ> f2 = f1a \<circ> f2a \<and> f1a \<in> Rep_rev_uspec S1 \<and> f2a \<in> Rep_rev_uspec (S2 \<circle> S3)"
        using assms(2) f2_def f3_def f4_def uspec_sercomp_bla1 by blast
    qed
  next
...
lemma uspec_parcompwell_asso2: assumes "uspec_parcompwell S1 S2" and "uspec_parcompwell S1 S3"
and "uspec_parcompwell S2 S3" 
...
lemma uspec_serrcompwell_asso_h: assumes "uspec_sercompwell S1 S2"
...
lemma uspec_serrcompwell_asso: assumes "uspec_sercompwell S1 S2"
and "uspec_sercompwell S2 S3"
and "uspecDom S1 \<inter> uspecRan S3 = {}"
shows "uspecWell  {f1 \<circ> f2 |f1 f2. f1 \<in> Rep_rev_uspec S1 \<and> f2 \<in> Rep_rev_uspec (S2 \<circle> S3)}"
  by (simp add: assms(1) assms(2) assms(3) uspec_serrcompwell_asso_h) 
