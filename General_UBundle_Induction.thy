(*  Title:        General_UBundle_Induction
    Author:       Janik

    Description:  Defines general UBundle induction
*)


theory General_UBundle_Induction

imports UBundle_Conc UnivClasses "untimed/Streams" 
...
(* ----------------------------------------------------------------------- *)
subsection\<open>ubTake lemmas\<close>
(* ----------------------------------------------------------------------- *)
...
lemma ubtake_zero: "ubTake 0\<cdot>In = ubLeast (ubDom\<cdot>In)"
  apply (simp add: ubtake_insert ubMapStream_def ubLeast_def)
  apply (subst usclTake_bot)
  by simp
