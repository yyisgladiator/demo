...
(* Warning: you should not use this in production, this is just a test, use the "normal" Stream *)
...
proof (rule lncases [of "#a"], simp)
  ...
  thus ?thesis by (rule Streams2.finind [of "a"], simp_all)
qed
...
lemma sdom_mono: "x\<sqsubseteq>y \<Longrightarrow> sdom x \<subseteq> sdom y"
...
text {* @{term sdom} is a continuous function *}
lemma sdom_def: "sdom\<cdot>s = {z. \<exists>n. Fin n < #s \<and> z = snth n s}"
...
lemma [simp]: "sdom \<epsilon> = {}"
by (auto simp add: sdom_def lnless_def)

(* the head of any stream is always an element of the domain *)
lemma sdom2un[simp]: "sdom (\<up>z \<bullet> s) = {z} \<union> sdom s"
apply (auto simp add: sdom_def)
apply (case_tac "n", auto)
apply (rule_tac x="0" in exI, auto)
by (rule_tac x="Suc n" in exI, auto)

(* only the empty stream has no elements in its domain *)
lemma strict_sdom_rev: "sdom s = {} \<Longrightarrow> s = \<epsilon>"
apply (auto simp add: sdom_def)
apply (rule_tac x="s" in scases, auto)
...
lemma [simp]: "sdom (sinftimes (\<up>a)) = {a}"
apply (auto simp add: sdom_def)
apply (induct_tac n, auto)
apply (subst sinftimes_unfold, simp)
apply (subst sinftimes_unfold, simp)
...
lemma sfilter_sdoml3:
  "sdom s \<subseteq> X \<longrightarrow> sfilter X\<cdot>s = s"
apply (rule impI)
apply (rule stream.take_lemma)
apply (simp add: atomize_imp)
apply (rule_tac x="s" in spec)
...
lemma sfilter_sdoml4 [simp]:
  "sfilter (sdom s)\<cdot>s = s"
by (simp add: sfilter_sdoml3)

text {* The domain of a concatenated stream is the union of the single domains *}
lemma sdom_sconc2un:
  "#x = Fin k \<Longrightarrow> sdom (x \<bullet> y) = sdom x \<union> sdom y"
apply (simp add: atomize_imp)
apply (rule_tac x="x" in spec)
apply (induct_tac k, simp+)
apply (rule allI, rule impI)
...
lemma sfilter2dom:
  "sfilter {z}\<cdot>x \<noteq> \<epsilon> \<Longrightarrow> z \<in> sdom x"
...
lemma sdom_smapl1: "\<lbrakk>x \<in> sdom (smap f\<cdot>s); inj f; f y = x\<rbrakk> \<Longrightarrow> y \<in> sdom s" 
apply (auto simp add: sdom_def)
apply (rule_tac x="n" in exI, simp)
